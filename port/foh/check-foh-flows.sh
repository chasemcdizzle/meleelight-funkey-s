...
LAUNCH_RE='^LAUNCH [0-9]+ p1=[0-4] p2=[0-4] p2type=[01] difficulty=[1-4] stage=[0-5] turbo=[01] lcancel=[012] flashlcancel=[01] walljump=[01] tapjump=[01],[01],[01],[01] versus=0 p3=[0-4] p4=[0-4] p3type=(-1|0|1) p4type=(-1|0|1) p3difficulty=[1-4] p4difficulty=[1-4]$'
...
for k in 0 1 2 3 4 5 6; do
  ...
  if [ "$mode" = "tverify" ]; then
    ...
    c="$(count_e "$B/$id/bstate.txt" '^BRIDGE-STATE p1=[0-4] p2=[0-4] p2type=[01] difficulty=[1-4] stage=[0-5] turbo=[01] lcancel=[012] tapjump=[01],[01],[01],[01] phantom=[0-9a-f]{16} p3=[0-4] p4=[0-4] p3type=(-1|0|1) p4type=(-1|0|1) p3difficulty=[1-4] p4difficulty=[1-4]$')"
...
witlaunch="LAUNCH 830 p1=$G01_P1 p2=$G01_P2 p2type=0 difficulty=3 stage=$G01_STAGE turbo=0 lcancel=1 flashlcancel=0 walljump=0 tapjump=0,0,0,0 versus=0 p3=0 p4=0 p3type=-1 p4type=-1 p3difficulty=3 p4difficulty=3"
...
ctrllaunch="LAUNCH 830 p1=$G01_P1 p2=$G01_P2 p2type=0 difficulty=3 stage=$G01_STAGE turbo=0 lcancel=0 flashlcancel=0 walljump=0 tapjump=0,0,0,0 versus=0 p3=0 p4=0 p3type=-1 p4type=-1 p3difficulty=3 p4difficulty=3"
...
LAUNCH_OK_LINE='LAUNCH GUARD WITNESS OK (cells=81 launch=26 refuse=55)'
...
launchwit=1
echo "    [5d] OK: all 81 (p1Type,p2Type,p3Type,p4Type) cells judged against the authored grid — 26 launch (every two-port-or-more configuration with a human port 0; A49 retired D40(b), so a CPU on port 2/3 is one of them, including P1+P3 with P2 off), 55 refuse with 'deny' + 'refused portconfig' and NO screen movement"
...
# T35: the guard WIDENED. RE-AUTHORED TWICE, and the second time is worth
# recording. A44 re-authored it because the two-port guard it originally
# narrowed no longer existed; it then dropped the arm refusing a CPU above
# port 1 (DEVIATION D40(b)). **A49 RETIRED D40(b) BY OWNER RULING**, so that
# arm is gone and the tooth had no subject left — a perturbation with nothing
# to perturb is a dead tooth, not a passing one.
#
# It moves to the guard's SURVIVING and now UNTOOTHED condition: PORT 0 MUST
# BE HUMAN (DEVIATION D17 on its amended ground — port 0 is the port the
# physical controller drives, so a CPU there would boot a match with no input
# source and make the LAUNCH record a lie, HARD RULE 2). Dropping that clause
# flips every grid cell where port 0 is N/A or CPU and two or more ports
# participate; each fails on all three of sound, event and screen. The count
# is MEASURED off the run, not derived: 46 cells, 138 failures.
rm -rf "$B/st35w"; mkdir -p "$B/st35w"
# `@` delimiter on purpose: the perturbed line sits inside a `||` chain.
sed 's@      if (s->portType\[0\] != 0 || participants < 2) {@      if (participants < 2) { // T35@' \
  "$FOH/foh.c" > "$B/st35w/foh.c"
[ "$(count_x "$B/st35w/foh.c" 's->portType[0] != 0 || participants < 2')" = 0 ] || fail "T35 — the widened-guard perturb did not take"
...
launch_tooth st35w "$B/st35w/w" "(p1=1,p2=1,p3=1,p4=1)" 138 \
  "T35 OK: a guard that stops requiring a HUMAN port 0 dies on the authored grid"
...
# 57, not 55: the 55 refusing cells each lose their event, and the two SHAPE
...
# It was 72 (70 + 2) until A49 retired D40(b) and 15 grid cells moved from
# refuse to launch; the DERIVATION is unchanged, only the grid it counts.
launch_tooth st36w "$B/st36w/w" "want exactly 1" 57 \
