...
m_sumbad() { sed -i.bak 's/^SUM \(.\)/SUM 0/' "$1/mlfk-match.hdr"; rm -f "$1"/*.bak; }
m_stage_unsealed() {
  ...
  sed -i.bak 's/^STAGE 00$/STAGE 03/' "$1/mlfk-match.hdr"; rm -f "$1"/*.bak
}
m_build_sealed() {
  sed -i.bak 's/^BUILD ./BUILD 0/' "$1/mlfk-match.hdr"; rm -f "$1"/*.bak
  reseal "$1/mlfk-match.hdr"
}
m_frame_sealed() {
  sed -i.bak 's/^FRAME 0*[0-9]\{4\}$/FRAME 000000001799/' "$1/mlfk-match.hdr"
  rm -f "$1"/*.bak
  reseal "$1/mlfk-match.hdr"
}
