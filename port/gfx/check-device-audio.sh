...
#      L) doubled pack verdict -> death; T9/T10 (iter 61, review-59 L)
...
# audio_judge_bytes_assert <judge-stdout-file> — FILE-BYTE terminator
# grammar (iter 61, review-59 L): $() command substitution strips ALL
# trailing newlines, so a string-level last-line check alone would
# silently normalize a judge output followed by trailing blank lines.
# This assert reads the file's actual bytes: it must END with EXACTLY
# the 17 bytes 'judge_complete=1\n' — one terminator line, one final
# newline, NOTHING after (the reference bytes are GENERATED by printf,
# never hand-transcribed). Trailing blank line(s), a missing final
# newline, or truncation = corruption death, never a retry.
audio_judge_bytes_assert() {
  local jf="$1"
  if ! tail -c 17 "$jf" | cmp -s - <(printf 'judge_complete=1\n'); then
    echo "DEVICE FAIL: audio judge output file $jf does not END with the exact bytes 'judge_complete=1<newline>' (trailing blank lines / missing final newline / truncation) — corrupt judge output, never a retry" >&2
    exit 1
  fi
}
...
# FILE-BYTE READ (iter 61, review-59 L): the judge's stdout is captured
# to a FILE and the integrity terminator is asserted on the file's
# BYTES before the (newline-stripping) $(cat) hands the content to the
# ingester — trailing blank lines violate the grammar as written.
parse_audio_judge() {
...
# pack_verdict_assert <pack-snd-stdout-FILE> — the packer verdict
# grammar (iter 59, review-57 L; FILE BYTES iter 61, review-59 L): the
# file must consist of EXACTLY one line matching the anchored full-line
# pattern $pack_line_re (set in step 2) followed by a single final
# newline — asserted BYTE-exactly (wc -c vs the matched line's length
# + 1), so trailing blank lines, extra bytes, doubled verdicts, or a
# missing final newline all violate the grammar as written ($() capture
# would have stripped the trailing-newline class silently).
pack_verdict_assert() {
  local pfile="$1" pcnt pline pbytes
  pcnt="$(grep -Ec "$pack_line_re" "$pfile")" || true
  if [ "$pcnt" != 1 ]; then
    echo "DEVICE FAIL: pack-snd output $pfile has $pcnt full-grammar verdict lines (want exactly 1 with count=${SND_COUNT_PIN}): '$(cat "$pfile")'" >&2
    exit 1
  fi
  pline="$(grep -E "$pack_line_re" "$pfile")"
  pbytes="$(wc -c < "$pfile" | tr -d ' ')"
  if [ "$pbytes" != "$((${#pline} + 1))" ]; then
    echo "DEVICE FAIL: pack-snd output $pfile is $pbytes bytes but the canonical form (the one verdict line + one final newline) is $((${#pline} + 1)) bytes — trailing/extra bytes violate the verdict grammar" >&2
    exit 1
  fi
}
...
for side in a b; do
  ...
  node "$GFX/pack-snd.js" "$AUDIO_OUT" "$BUILD/sndpack-$side.bin" \
    > "$BUILD/pack-out-$side.txt" || {
    echo "DEVICE FAIL: pack-snd.js failed (side $side)" >&2
    exit 1
  }
  made "$BUILD/pack-out-$side.txt"
  # iter 59 L exactly-one full line; iter 61 review-59 L: FILE bytes
  pack_verdict_assert "$BUILD/pack-out-$side.txt"
...
# T8 (iter 59, review-57 L; file-based since iter 61): a doubled pack
# verdict — as FILE bytes — must die.
rm -f "$BUILD/t8-pack.txt"
printf 'pack-snd OK count=%s dataBytes=1 fileBytes=1\npack-snd OK count=%s dataBytes=1 fileBytes=1\n' \
  "$SND_COUNT_PIN" "$SND_COUNT_PIN" > "$BUILD/t8-pack.txt"
made "$BUILD/t8-pack.txt"
t8rc=0
( pack_verdict_assert "$BUILD/t8-pack.txt" ) >/dev/null 2>&1 || t8rc=$?
if [ "$t8rc" -eq 0 ]; then
  echo "DEVICE FAIL: T8 tooth — doubled pack-snd verdict was ACCEPTED" >&2
  exit 1
fi
# T9 (iter 61, review-59 L): a REAL judge output with ONE trailing
# blank line appended must die in the FILE-BYTE terminator assert —
# $()'s trailing-newline stripping must not normalize it. Positive
# control first: the clean judge output passes the byte assert.
rm -f "$BUILD/t9-judge.txt"
t9rc=0
node "$GFX/judge-audio-summary.js" "$BUILD/g01.au-log-a.txt" \
  --rate 0 --samples 0 --channels 0 --cbs-min 0 --cbs-max 0 \
  --max-underruns 0 --max-badlen 0 > "$BUILD/t9-judge.txt" || t9rc=$?
if [ "$t9rc" -ne 0 ]; then
  echo "DEVICE FAIL: T9 positive control — judge on the clean host log must exit 0 (rc $t9rc)" >&2
  exit 1
fi
audio_judge_bytes_assert "$BUILD/t9-judge.txt" # positive control: clean bytes PASS
printf '\n' >> "$BUILD/t9-judge.txt"
t9brc=0
( audio_judge_bytes_assert "$BUILD/t9-judge.txt" ) >/dev/null 2>&1 || t9brc=$?
if [ "$t9brc" -eq 0 ]; then
  echo "DEVICE FAIL: T9 tooth — judge output with a trailing blank line was ACCEPTED by the file-byte terminator assert" >&2
  exit 1
fi
# T10 (iter 61, review-59 L): the REAL pack verdict file with ONE
# trailing blank line appended must die in the byte-exact verdict
# grammar (the a-side file already passed it clean in step 2).
rm -f "$BUILD/t10-pack.txt"
cp "$BUILD/pack-out-a.txt" "$BUILD/t10-pack.txt"
printf '\n' >> "$BUILD/t10-pack.txt"
made "$BUILD/t10-pack.txt"
t10rc=0
( pack_verdict_assert "$BUILD/t10-pack.txt" ) >/dev/null 2>&1 || t10rc=$?
if [ "$t10rc" -eq 0 ]; then
  echo "DEVICE FAIL: T10 tooth — pack verdict with a trailing blank line was ACCEPTED" >&2
  exit 1
fi
echo "   standing teeth OK (T1 truncation death, T2 dropped-blob death at play, T3 underrun leg fires, T4 grammar deaths x2, T6 truncated-judge corruption death, T7 torn-summary death, T8 doubled-pack-verdict death, T9 judge-trailing-blank byte death, T10 pack-trailing-blank byte death)"
