// THROWNFOXBACK.c <- src/characters/falcon/moves/THROWNFOXBACK.js (M2 task 10; falcon-vs-fox diff is DATA-ONLY, structure verbatim from the task-8 translation)
#include "../moves.h"
...
static AsTri fc4_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc4_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_falcon_arr_len("THROWNFOXBACK", "offset")) {
        timer = mv_falcon_arr_len("THROWNFOXBACK", "offset") - 1;
      }
      Vec2D off;
      if (!mv_falcon_pair("THROWNFOXBACK", "offset", timer - 1, &off)) {
...
const MlMoveDef falcon_THROWNFOXBACK = {"THROWNFOXBACK", fc4_init, fc4_main,
                                     fc4_interrupt, 0};
