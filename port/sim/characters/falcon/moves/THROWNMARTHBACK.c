// THROWNMARTHBACK.c <- src/characters/falcon/moves/THROWNMARTHBACK.js
...
static AsTri fc4_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc4_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_falcon_arr_len("THROWNMARTHBACK", "offset")) {
        timer = mv_falcon_arr_len("THROWNMARTHBACK", "offset") - 1;
      }
      Vec2D off;
      if (!mv_falcon_pair("THROWNMARTHBACK", "offset", timer - 1, &off)) {
...
const MlMoveDef falcon_THROWNMARTHBACK = {"THROWNMARTHBACK", fc4_init, fc4_main,
                                       fc4_interrupt, 0};
