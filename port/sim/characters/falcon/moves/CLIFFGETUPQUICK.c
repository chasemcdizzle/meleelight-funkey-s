// CLIFFGETUPQUICK.c <- src/characters/falcon/moves/CLIFFGETUPQUICK.js
...
static AsTri fc4_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc4_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (pl->timer < 20) {
      Vec2D off;
      if (!mv_falcon_pair("CLIFFGETUPQUICK", "offset", pl->timer - 1, &off)) {
        mv_out_of_domain("CLIFFGETUPQUICK: offset index out of range");
      }
      mv_pos_reassign(S, p, vec2d(lp.x + (off.x + 68.4) * pl->phys.face,
                                  lp.y + off.y));
    } else {
      pl->phys.cVel.x = mv_falcon_arr("CLIFFGETUPQUICK", "setVelocities",
                                      pl->timer - 20) * pl->phys.face;
    }
    if (pl->timer == 20) {
...
const MlMoveDef falcon_CLIFFGETUPQUICK = {"CLIFFGETUPQUICK", fc4_init, fc4_main,
                                fc4_interrupt, 0};
