// CLIFFGETUPSLOW.c <- src/characters/falcon/moves/CLIFFGETUPSLOW.js
...
static AsTri fc4_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc4_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (pl->timer < 38) {
      Vec2D off;
      if (!mv_falcon_pair("CLIFFGETUPSLOW", "offset", pl->timer - 1, &off)) {
        mv_out_of_domain("CLIFFGETUPSLOW: offset index out of range");
      }
      mv_pos_reassign(S, p, vec2d(lp.x + (off.x + 68.4) * pl->phys.face,
                                  lp.y + off.y));
    } else {
      pl->phys.cVel.x = mv_falcon_arr("CLIFFGETUPSLOW", "setVelocities",
...
const MlMoveDef falcon_CLIFFGETUPSLOW = {"CLIFFGETUPSLOW", fc4_init, fc4_main,
                                fc4_interrupt, 0};
