// THROWNFOXDOWN.c <- src/characters/falcon/moves/THROWNFOXDOWN.js (M2 task 10; falcon-vs-fox diff is DATA-ONLY, structure verbatim from the task-8 translation)
#include "../moves.h"
...
static AsTri fc4_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc4_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_falcon_arr_len("THROWNFOXDOWN", "offset")) {
        timer = mv_falcon_arr_len("THROWNFOXDOWN", "offset") - 1;
      }
      Vec2D off;
      if (!mv_falcon_pair("THROWNFOXDOWN", "offset", timer - 1, &off)) {
...
const MlMoveDef falcon_THROWNFOXDOWN = {"THROWNFOXDOWN", fc4_init, fc4_main,
                                     fc4_interrupt, 0};
