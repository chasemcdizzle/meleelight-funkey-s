// THROWNFALCOUP.c <- src/characters/falcon/moves/THROWNFALCOUP.js (M2 task 10; falcon-vs-fox diff is DATA-ONLY, structure verbatim from the task-8 translation)
...
// would THROW there — mv_player / the mv_falcon_pair trap mirror that).
#include "../moves.h"
...
static AsTri fc4_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc4_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      Vec2D off;
      if (!mv_falcon_pair("THROWNFALCOUP", "offset", pl->timer - 1, &off)) {
...
const MlMoveDef falcon_THROWNFALCOUP = {"THROWNFALCOUP", fc4_init, fc4_main,
                                     fc4_interrupt, 0};
