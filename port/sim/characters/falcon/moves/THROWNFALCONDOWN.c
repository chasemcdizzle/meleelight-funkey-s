// THROWNFALCONDOWN.c <- src/characters/falcon/moves/THROWNFALCONDOWN.js
// (M2 task 10; falcon-vs-fox diff is DATA-ONLY, structure verbatim from the task-8 translation). Unguarded family; NO face flip upstream (unlike
...
static AsTri fc4_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc4_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      Vec2D off;
      if (!mv_falcon_pair("THROWNFALCONDOWN", "offset", pl->timer - 1, &off)) {
...
const MlMoveDef falcon_THROWNFALCONDOWN = {"THROWNFALCONDOWN", fc4_init, fc4_main,
                                        fc4_interrupt, 0};
