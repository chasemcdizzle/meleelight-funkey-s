// THROWNFALCONFORWARD.c <- src/characters/falcon/moves/THROWNFALCONFORWARD.js
...
static AsTri fc4_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc4_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      Vec2D off;
      if (!mv_falcon_pair("THROWNFALCONFORWARD", "offset", pl->timer - 1,
                       &off)) {
...
const MlMoveDef falcon_THROWNFALCONFORWARD = {"THROWNFALCONFORWARD", fc4_init,
