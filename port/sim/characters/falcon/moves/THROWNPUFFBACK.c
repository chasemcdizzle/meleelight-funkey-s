// THROWNPUFFBACK.c <- src/characters/falcon/moves/THROWNPUFFBACK.js
// (M2 task 10; falcon-vs-fox diff is DATA-ONLY, structure verbatim from the task-8 translation). face *= -1 in init; no pos assignment in init; main's pos
// x-offset multiplies face * -1. offsetVel is dead data (never read).
#include "../moves.h"
...
static AsTri fc4_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc4_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_falcon_arr_len("THROWNPUFFBACK", "offset")) {
        timer = mv_falcon_arr_len("THROWNPUFFBACK", "offset") - 1;
      }
      Vec2D off;
      if (!mv_falcon_pair("THROWNPUFFBACK", "offset", timer - 1, &off)) {
...
const MlMoveDef falcon_THROWNPUFFBACK = {"THROWNPUFFBACK", fc4_init, fc4_main,
                                      fc4_interrupt, 0};
