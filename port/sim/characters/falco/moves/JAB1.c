// JAB1.c <- src/characters/falco/moves/JAB1.js (M2 task 9)
#include "../moves.h"
...
static AsTri fc_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (pl->timer > 2 && pl->timer < 32 && i0->a && !i1->a) {
...
const MlMoveDef falco_JAB1 = {"JAB1", fc_init, fc_main, fc_interrupt, 0};
