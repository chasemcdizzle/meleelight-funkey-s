// DOWNSPECIALAIREND.c <- src/characters/falco/moves/DOWNSPECIALAIREND.js
// (M2 task 9)
#include "../moves.h"
...
static AsTri fc_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  strcpy(pl->actionState, "DOWNSPECIALAIREND");
  pl->timer = 0;
...
static AsTri fc_land(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  (void)in; (void)ex;
  MlPlayer *pl = mv_player(S, p);
  strcpy(pl->actionState, "DOWNSPECIALGROUNDEND");
  return AS_UNDEF;
}

const MlMoveDef falco_DOWNSPECIALAIREND = {"DOWNSPECIALAIREND", fc_init,
