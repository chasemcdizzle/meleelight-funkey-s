  ...
  if (fc_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (prevFrame < 23 && pl->timer >= 23) {
      ...
      ml_drawVfx_laser("laser", pl->phys.pos.x + (1 * pl->phys.face),
                       pl->phys.pos.y + 12, pl->phys.face,
                       js_pi() * 275 / 180, 137, 255, 255, 157, 255, 255);
    } else if (prevFrame < 25 && pl->timer >= 25) {
      mv_article_laser_falco(S, p, 1, 16, js_pi() * 260 / 180, true);
      ml_sound_play("foxlaserfire");
      // 260
      ml_drawVfx_laser("laser", pl->phys.pos.x + (1 * pl->phys.face),
                       pl->phys.pos.y + 16, pl->phys.face,
                       js_pi() * 260 / 180, 137, 255, 255, 157, 255, 255);
      ...
      ml_drawVfx_laser("laser", pl->phys.pos.x + (2 * pl->phys.face),
                       pl->phys.pos.y + 15, pl->phys.face,
                       js_pi() * 290 / 180, 137, 255, 255, 157, 255, 255);
    } else if (prevFrame < 31 && pl->timer >= 31) {
      mv_article_laser_falco(S, p, 2, 17, js_pi() * 275 / 180, true);
      ml_sound_play("foxlaserfire");
      // 275
      ml_drawVfx_laser("laser", pl->phys.pos.x + (2 * pl->phys.face),
                       pl->phys.pos.y + 17, pl->phys.face,
                       js_pi() * 275 / 180, 137, 255, 255, 157, 255, 255);
    }
  }
  return AS_UNDEF;
