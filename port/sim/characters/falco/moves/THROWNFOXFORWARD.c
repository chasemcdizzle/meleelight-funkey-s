// THROWNFOXFORWARD.c <- src/characters/falco/moves/THROWNFOXFORWARD.js (M2 task 9)
...
static AsTri fc_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fc_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      Vec2D off;
      if (!mv_falco_pair("THROWNFOXFORWARD", "offset", pl->timer - 1, &off)) {
...
const MlMoveDef falco_THROWNFOXFORWARD = {"THROWNFOXFORWARD", fc_init, fc_main, fc_interrupt,
    0};
