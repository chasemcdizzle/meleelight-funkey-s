// THROWNPUFFBACK.c <- src/characters/fox/moves/THROWNPUFFBACK.js
// (M2 task 8). face *= -1 in init; no pos assignment in init; main's pos
...
static AsTri fx_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fx_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_fox_arr_len("THROWNPUFFBACK", "offset")) {
        timer = mv_fox_arr_len("THROWNPUFFBACK", "offset") - 1;
      }
      Vec2D off;
      if (!mv_fox_pair("THROWNPUFFBACK", "offset", timer - 1, &off)) {
...
const MlMoveDef fox_THROWNPUFFBACK = {"THROWNPUFFBACK", fx_init, fx_main,
                                      fx_interrupt, 0};
