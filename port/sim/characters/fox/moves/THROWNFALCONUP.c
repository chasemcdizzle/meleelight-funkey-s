// THROWNFALCONUP.c <- src/characters/fox/moves/THROWNFALCONUP.js
// (M2 task 8). Unguarded family — see THROWNFALCOUP.c.
#include "../moves.h"
...
static AsTri fx_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fx_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      Vec2D off;
      if (!mv_fox_pair("THROWNFALCONUP", "offset", pl->timer - 1, &off)) {
...
const MlMoveDef fox_THROWNFALCONUP = {"THROWNFALCONUP", fx_init, fx_main,
                                      fx_interrupt, 0};
