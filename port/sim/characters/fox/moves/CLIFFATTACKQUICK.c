// CLIFFATTACKQUICK.c <- src/characters/fox/moves/CLIFFATTACKQUICK.js
...
static AsTri fx_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fx_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (pl->timer < 24) {
      if (pl->timer >= 14) {
        Vec2D off;
        if (!mv_fox_pair("CLIFFATTACKQUICK", "offset", pl->timer - 14,
                         &off)) {
      ...
      pl->phys.cVel.x = mv_fox_arr("CLIFFATTACKQUICK", "setVelocities",
...
const MlMoveDef fox_CLIFFATTACKQUICK = {"CLIFFATTACKQUICK", fx_init, fx_main,
                                        fx_interrupt, 0};
