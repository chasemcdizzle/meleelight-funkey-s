// THROWNMARTHUP.c <- src/characters/fox/moves/THROWNMARTHUP.js (M2 task 8)
#include "../moves.h"
...
static AsTri fx_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fx_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_fox_arr_len("THROWNMARTHUP", "offset")) {
        timer = mv_fox_arr_len("THROWNMARTHUP", "offset") - 1;
      }
      Vec2D off;
      if (!mv_fox_pair("THROWNMARTHUP", "offset", timer - 1, &off)) {
...
const MlMoveDef fox_THROWNMARTHUP = {"THROWNMARTHUP", fx_init, fx_main,
                                     fx_interrupt, 0};
