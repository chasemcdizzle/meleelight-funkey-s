// THROWNPUFFUP.c <- src/characters/fox/moves/THROWNPUFFUP.js (M2 task 8)
...
static AsTri fx_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fx_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      ...
      if (grabbedBy != -1) {
        if (pl->timer > mv_fox_arr_len("THROWNPUFFUP", "offset")) {
          pl->timer = mv_fox_arr_len("THROWNPUFFUP", "offset") - 1;
        }
        Vec2D off;
        if (!mv_fox_pair("THROWNPUFFUP", "offset", pl->timer - 1, &off)) {
...
const MlMoveDef fox_THROWNPUFFUP = {"THROWNPUFFUP", fx_init, fx_main,
                                    fx_interrupt, 0};
