// SIDESPECIALAIR.c <- src/characters/puff/moves/SIDESPECIALAIR.js (M2 task 12)
...
// lsY * PI * (20/180) via fdlibm sin/cos. Land is a bare actionState write (no init call).
#include "../moves.h"
...
static AsTri pf_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->phys.grounded) {
      if (pl->timer > 11) {
        pl->phys.cVel.x = mv_puff_arr("SIDESPECIALAIR", "groundVelocities",
      ...
      if (pl->timer < 12) {
        ...
        pl->phys.cVel.x = mv_puff_arr("SIDESPECIALAIR", "airVelocities",
                                      pl->timer - 12) *
                          pl->phys.face * fd_cos(pl->phys.upbAngleMultiplier);
        pl->phys.cVel.y = mv_puff_arr("SIDESPECIALAIR", "airVelocities",
                                      pl->timer - 12) *
...
const MlMoveDef puff_SIDESPECIALAIR = {"SIDESPECIALAIR", pf_init, pf_main,
