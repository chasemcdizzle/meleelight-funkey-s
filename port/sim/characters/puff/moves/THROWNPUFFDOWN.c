// THROWNPUFFDOWN.c <- src/characters/puff/moves/THROWNPUFFDOWN.js (M2 task 12)
...
static AsTri pf_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_puff_arr_len("THROWNPUFFDOWN", "offset")) {
        timer = mv_puff_arr_len("THROWNPUFFDOWN", "offset") - 1;
      }
        Vec2D off;
        if (!mv_puff_pair("THROWNPUFFDOWN", "offset", timer - 1, &off)) {
...
const MlMoveDef puff_THROWNPUFFDOWN = {"THROWNPUFFDOWN", pf_init, pf_main, pf_interrupt, 0};
