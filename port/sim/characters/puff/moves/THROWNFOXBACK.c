// THROWNFOXBACK.c <- src/characters/puff/moves/THROWNFOXBACK.js (M2 task 12)
// Guarded family; init flips face, x uses face*-1.
#include "../moves.h"
...
static AsTri pf_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_puff_arr_len("THROWNFOXBACK", "offset")) {
        timer = mv_puff_arr_len("THROWNFOXBACK", "offset") - 1;
      }
        Vec2D off;
        if (!mv_puff_pair("THROWNFOXBACK", "offset", timer - 1, &off)) {
...
const MlMoveDef puff_THROWNFOXBACK = {"THROWNFOXBACK", pf_init, pf_main, pf_interrupt, 0};
