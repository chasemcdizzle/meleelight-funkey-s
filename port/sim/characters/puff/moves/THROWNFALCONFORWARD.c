// THROWNFALCONFORWARD.c <- src/characters/puff/moves/THROWNFALCONFORWARD.js (M2 task 12)
// Unguarded family.
#include "../moves.h"
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      ...
      const double grabbedBy = pl->phys.grabbedBy;
      {
        ...
        Vec2D off;
        if (!mv_puff_pair("THROWNFALCONFORWARD", "offset", timer - 1, &off)) {
...
const MlMoveDef puff_THROWNFALCONFORWARD = {"THROWNFALCONFORWARD", pf_init, pf_main, pf_interrupt, 0};
