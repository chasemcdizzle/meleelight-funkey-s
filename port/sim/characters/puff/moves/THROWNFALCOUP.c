// THROWNFALCOUP.c <- src/characters/puff/moves/THROWNFALCOUP.js (M2 task 12)
// Old-style UNGUARDED family (`this.`-dispatch upstream): init pos snap, no -1 guards, no clamps (player[-1]/overruns throw — traps).
#include "../moves.h"
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      ...
      const double grabbedBy = pl->phys.grabbedBy;
      {
        ...
        Vec2D off;
        if (!mv_puff_pair("THROWNFALCOUP", "offset", timer - 1, &off)) {
...
const MlMoveDef puff_THROWNFALCOUP = {"THROWNFALCOUP", pf_init, pf_main, pf_interrupt, 0};
