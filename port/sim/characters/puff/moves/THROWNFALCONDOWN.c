// THROWNFALCONDOWN.c <- src/characters/puff/moves/THROWNFALCONDOWN.js (M2 task 12)
// Unguarded family; carries reverseModel but NO face flip and PLAIN-face x — verbatim quirk (unlike THROWNFALCODOWN).
#include "../moves.h"
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      ...
      const double grabbedBy = pl->phys.grabbedBy;
      {
        ...
        Vec2D off;
        if (!mv_puff_pair("THROWNFALCONDOWN", "offset", timer - 1, &off)) {
...
const MlMoveDef puff_THROWNFALCONDOWN = {"THROWNFALCONDOWN", pf_init, pf_main, pf_interrupt, 0};
