// CLIFFJUMPQUICK.c <- src/characters/puff/moves/CLIFFJUMPQUICK.js (M2 task 12)
// pos t<15, cVel=(1.1*face, 1.8) at t===15, drift+fastfall after; exits to FALL.
#include "../moves.h"
...
static AsTri pf_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    const Vec2D lp = mv_ledge_point(S, onLedge, "CLIFFJUMPQUICK: ledge deref");
    if (pl->timer < 15) {
      Vec2D off;
      if (!mv_puff_pair("CLIFFJUMPQUICK", "offset", pl->timer - 1, &off)) {
...
const MlMoveDef puff_CLIFFJUMPQUICK = {"CLIFFJUMPQUICK", pf_init, pf_main, pf_interrupt, 0};
