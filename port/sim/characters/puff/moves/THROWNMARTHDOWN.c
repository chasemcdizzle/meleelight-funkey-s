// THROWNMARTHDOWN.c <- src/characters/puff/moves/THROWNMARTHDOWN.js (M2 task 12)
// Guarded family; init snap + face flip, x uses face*-1.
#include "../moves.h"
...
static AsTri pf_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_puff_arr_len("THROWNMARTHDOWN", "offset")) {
        timer = mv_puff_arr_len("THROWNMARTHDOWN", "offset") - 1;
      }
        Vec2D off;
        if (!mv_puff_pair("THROWNMARTHDOWN", "offset", timer - 1, &off)) {
...
const MlMoveDef puff_THROWNMARTHDOWN = {"THROWNMARTHDOWN", pf_init, pf_main, pf_interrupt, 0};
