// THROWNFALCOFORWARD.c <- src/characters/puff/moves/THROWNFALCOFORWARD.js (M2 task 12)
// Unguarded family; offsets carry authored expressions (evaluated by the mvData dump — rule 15).
#include "../moves.h"
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      ...
      const double grabbedBy = pl->phys.grabbedBy;
      {
        ...
        Vec2D off;
        if (!mv_puff_pair("THROWNFALCOFORWARD", "offset", timer - 1, &off)) {
...
const MlMoveDef puff_THROWNFALCOFORWARD = {"THROWNFALCOFORWARD", pf_init, pf_main, pf_interrupt, 0};
