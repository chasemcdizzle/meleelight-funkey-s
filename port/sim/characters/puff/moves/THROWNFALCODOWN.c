// THROWNFALCODOWN.c <- src/characters/puff/moves/THROWNFALCODOWN.js (M2 task 12)
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      ...
      const double grabbedBy = pl->phys.grabbedBy;
      {
        ...
        Vec2D off;
        if (!mv_puff_pair("THROWNFALCODOWN", "offset", timer - 1, &off)) {
...
const MlMoveDef puff_THROWNFALCODOWN = {"THROWNFALCODOWN", pf_init, pf_main, pf_interrupt, 0};
