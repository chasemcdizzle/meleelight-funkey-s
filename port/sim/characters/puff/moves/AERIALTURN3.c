// AERIALTURN3.c <- src/characters/puff/moves/AERIALTURN3.js (M2 task 12)
// The lsX-away multijump rung: flips face at t===6, hands off to JUMPAERIAL3 at t===13.
#include "../moves.h"
...
static AsTri pf_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  strcpy(pl->actionState, "AERIALTURN3");
  pl->timer = 0;
...
static AsTri pf_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pl->timer == 13) {
    pl->timer -= 1;
    strcpy(pl->actionState, "JUMPAERIAL3");
    puff_JUMPAERIAL3.main_(S, p, in, 0);
  } else {
...
const MlMoveDef puff_AERIALTURN3 = {"AERIALTURN3", pf_init, pf_main, pf_interrupt, 0};
