// DOWNTILT.c <- src/characters/puff/moves/DOWNTILT.js (M2 task 12)
// Interrupt block carries INLINE tiltturn/walk conditions (no checkForTiltTurn/plain-walk calls) — verbatim.
#include "../moves.h"
...
static AsTri pf_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  pf_assign_hitbox_id(S, p, "dtilt", 0, 0);
  pf_assign_hitbox_id(S, p, "dtilt", 1, 1);
  pf_assign_hitbox_id(S, p, "dtilt", 2, 2);
  pf_main(S, p, in, 0);
  return AS_UNDEF;
}
...
const MlMoveDef puff_DOWNTILT = {"DOWNTILT", pf_init, pf_main, pf_interrupt, 0};
