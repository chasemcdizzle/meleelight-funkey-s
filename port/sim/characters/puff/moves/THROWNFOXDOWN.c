// THROWNFOXDOWN.c <- src/characters/puff/moves/THROWNFOXDOWN.js (M2 task 12)
...
static AsTri pf_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_puff_arr_len("THROWNFOXDOWN", "offset")) {
        timer = mv_puff_arr_len("THROWNFOXDOWN", "offset") - 1;
      }
        Vec2D off;
        if (!mv_puff_pair("THROWNFOXDOWN", "offset", timer - 1, &off)) {
...
const MlMoveDef puff_THROWNFOXDOWN = {"THROWNFOXDOWN", pf_init, pf_main, pf_interrupt, 0};
