// AERIALTURN5.c <- src/characters/puff/moves/AERIALTURN5.js (M2 task 12)
// The lsX-away multijump rung: flips face at t===6, hands off to JUMPAERIAL5 at t===13.
#include "../moves.h"
...
static AsTri pf_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  strcpy(pl->actionState, "AERIALTURN5");
  pl->timer = 0;
...
static AsTri pf_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pl->timer == 13) {
    pl->timer -= 1;
    strcpy(pl->actionState, "JUMPAERIAL5");
    puff_JUMPAERIAL5.main_(S, p, in, 0);
  } else {
...
const MlMoveDef puff_AERIALTURN5 = {"AERIALTURN5", pf_init, pf_main, pf_interrupt, 0};
