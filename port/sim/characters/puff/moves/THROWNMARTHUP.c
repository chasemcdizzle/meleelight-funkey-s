// THROWNMARTHUP.c <- src/characters/puff/moves/THROWNMARTHUP.js (M2 task 12)
// Guarded family; init snaps pos to the grabber (marth-victim files only).
#include "../moves.h"
...
static AsTri pf_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pf_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      ...
      if (timer > mv_puff_arr_len("THROWNMARTHUP", "offset")) {
        timer = mv_puff_arr_len("THROWNMARTHUP", "offset") - 1;
      }
        Vec2D off;
        if (!mv_puff_pair("THROWNMARTHUP", "offset", timer - 1, &off)) {
...
const MlMoveDef puff_THROWNMARTHUP = {"THROWNMARTHUP", pf_init, pf_main, pf_interrupt, 0};
