// AERIALTURN1.c <- src/characters/puff/moves/AERIALTURN1.js (M2 task 12)
// The lsX-away multijump rung: flips face at t===6, hands off to JUMPAERIAL1 at t===13.
#include "../moves.h"
...
static AsTri pf_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  strcpy(pl->actionState, "AERIALTURN1");
  pl->timer = 0;
...
static AsTri pf_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pl->timer == 13) {
    pl->timer -= 1;
    strcpy(pl->actionState, "JUMPAERIAL1");
    puff_JUMPAERIAL1.main_(S, p, in, 0);
  } else {
...
const MlMoveDef puff_AERIALTURN1 = {"AERIALTURN1", pf_init, pf_main, pf_interrupt, 0};
