// DEADUP.c <- src/characters/shared/moves/DEADUP.js (M2 task 7)
#include "../moves.h"
...
static AsTri mv_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  ml_sound_play("kill");
  mv_dispatch(S, MV_CS(S, p), "DEADUP", "main", p, in, 0);
  return AS_UNDEF;
}
...
static AsTri mv_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (mv_dispatch(S, MV_CS(S, p), "DEADUP", "interrupt", p, in, 0) !=
      AS_TRUE) {
    ...
    if (pl->timer == 4) {
      if (mv_isFinalDeath(S)) {
        ...
        mv_out_of_domain("DEADUP: finishGame (final death)");
      } else {
        mv_screenShake();
...
const MlMoveDef mv_DEADUP = {"DEADUP", mv_init, mv_main, mv_interrupt, 0};
