// SMASHTURN.c <- src/characters/shared/moves/SMASHTURN.js (M2 task 7)
#include "../moves.h"
...
static AsTri mv_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  strcpy(pl->actionState, "SMASHTURN");
  pl->timer = 0;
  pl->phys.face *= -1;
  mv_dispatch(S, MV_CS(S, p), "SMASHTURN", "main", p, in, 0);
  return AS_UNDEF;
}
...
static AsTri mv_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  MlPlayer *pl = mv_player(S, p);
  pl->timer += 1;
  if (mv_dispatch(S, MV_CS(S, p), "SMASHTURN", "interrupt", p, in, 0) !=
      AS_TRUE) {
...
static AsTri mv_interrupt(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (j.flag) {
    ...
    return AS_TRUE;
  } else if (pl->timer == 2 && i0->lsX * pl->phys.face > 0.79) {
...
const MlMoveDef mv_SMASHTURN = {"SMASHTURN", mv_init, mv_main, mv_interrupt,
                                0};
