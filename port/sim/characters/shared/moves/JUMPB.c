// JUMPB.c <- src/characters/shared/moves/JUMPB.js (M2 task 7)
#include "../moves.h"
...
static AsTri mv_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  ml_sound_play("jump2");
  mv_dispatch(S, MV_CS(S, p), "JUMPB", "main", p, in, 0);
  return AS_UNDEF;
}
...
static AsTri mv_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  MlPlayer *pl = mv_player(S, p);
  pl->timer += 1;
  if (mv_dispatch(S, MV_CS(S, p), "JUMPB", "interrupt", p, in, 0) != AS_TRUE) {
    if (pl->timer > 1) {
...
static AsTri mv_interrupt(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (a.flag) {
    ...
    return AS_TRUE;
  } else if (pl->timer > mv_frames(MV_CS(S, p), "JUMPB")) {
...
const MlMoveDef mv_JUMPB = {"JUMPB", mv_init, mv_main, mv_interrupt, 0};
