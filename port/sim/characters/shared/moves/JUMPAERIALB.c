// JUMPAERIALB.c <- src/characters/shared/moves/JUMPAERIALB.js (M2 task 7)
#include "../moves.h"
...
static AsTri mv_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  strcpy(pl->actionState, "JUMPAERIALB");
  pl->timer = 0;
...
static AsTri mv_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (mv_dispatch(S, MV_CS(S, p), "JUMPAERIALB", "interrupt", p, in, 0) !=
      AS_TRUE) {
...
static AsTri mv_interrupt(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (a.flag) {
    ...
    return AS_TRUE;
  } else if (pl->timer > mv_frames(MV_CS(S, p), "JUMPAERIALB")) {
...
const MlMoveDef mv_JUMPAERIALB = {"JUMPAERIALB", mv_init, mv_main,
                                  mv_interrupt, 0};
