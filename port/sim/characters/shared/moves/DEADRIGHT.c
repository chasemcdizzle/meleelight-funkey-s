// DEADRIGHT.c <- src/characters/shared/moves/DEADRIGHT.js (M2 task 7)
#include "../moves.h"
...
static AsTri mv_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  strcpy(pl->actionState, "DEADRIGHT");
  pl->timer = 0;
  pl->phys.cVel.x = 0;
  pl->phys.cVel.y = 0;
  pl->phys.kVel.x = 0;
  pl->phys.kVel.y = 0;
  pl->percent = 0;
  ml_drawVfx("blastzoneExplosion", pl->phys.pos.x, pl->phys.pos.y, -90);
  if (!mv_isFinalDeath(S)) {
    mv_screenShake();
    // percentShake: native-RNG HUD shake (CHECKSUM.md §7) — no-op
  }
  ml_sound_play("kill");
  mv_dispatch(S, MV_CS(S, p), "DEADRIGHT", "main", p, in, 0);
  return AS_UNDEF;
}
...
static AsTri mv_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (mv_dispatch(S, MV_CS(S, p), "DEADRIGHT", "interrupt", p, in, 0) !=
      AS_TRUE) {
...
const MlMoveDef mv_DEADRIGHT = {"DEADRIGHT", mv_init, mv_main, mv_interrupt, 0};
