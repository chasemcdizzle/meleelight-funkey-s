// SQUATRV.c <- src/characters/shared/moves/SQUATRV.js (M2 task 7)
#include "../moves.h"
...
static AsTri mv_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  strcpy(pl->actionState, "SQUATRV");
  pl->timer = 0;
  mv_dispatch(S, MV_CS(S, p), "SQUATRV", "main", p, in, 0);
  return AS_UNDEF;
}
...
static AsTri mv_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  MlPlayer *pl = mv_player(S, p);
  pl->timer += 1;
  if (mv_dispatch(S, MV_CS(S, p), "SQUATRV", "interrupt", p, in, 0) !=
      AS_TRUE) {
...
static AsTri mv_interrupt(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (pl->timer > mv_frames(MV_CS(S, p), "SQUATRV")) {
...
const MlMoveDef mv_SQUATRV = {"SQUATRV", mv_init, mv_main, mv_interrupt, 0};
