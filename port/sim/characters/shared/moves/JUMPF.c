// JUMPF.c <- src/characters/shared/moves/JUMPF.js (M2 task 7)
#include "../moves.h"
...
static AsTri mv_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  strcpy(pl->actionState, "JUMPF");
  pl->timer = 0;
  if (mvx_truthy(ex, 0)) {
    pl->phys.cVel.y += ml_f64(at->fHopInitV);
  } else {
    pl->phys.cVel.y += ml_f64(at->sHopInitV);
  }

  pl->phys.cVel.x = (pl->phys.cVel.x * ml_f64(at->groundToAir)) +
                    (i0->lsX * ml_f64(at->jumpHinitV));
  if (js_abs(pl->phys.cVel.x) > ml_f64(at->jumpHmaxV)) {
    pl->phys.cVel.x = ml_f64(at->jumpHmaxV) * js_sign(pl->phys.cVel.x);
  }

  pl->phys.grounded = false;
  ml_sound_play("jump2");
  mv_dispatch(S, MV_CS(S, p), "JUMPF", "main", p, in, 0);
  return AS_UNDEF;
}
...
static AsTri mv_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  MlPlayer *pl = mv_player(S, p);
  pl->timer += 1;
  mv_playSounds(S, "JUMP", p);
  if (mv_dispatch(S, MV_CS(S, p), "JUMPF", "interrupt", p, in, 0) != AS_TRUE) {
    if (pl->timer > 1) {
...
static AsTri mv_interrupt(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (a.flag) {
    ...
    return AS_TRUE;
  } else if (pl->timer > mv_frames(MV_CS(S, p), "JUMPF")) {
...
const MlMoveDef mv_JUMPF = {"JUMPF", mv_init, mv_main, mv_interrupt, 0};
