// THROWNFOXFORWARD.c <- src/characters/marth/moves/THROWNFOXFORWARD.js (M2 task 11).
...
static AsTri mr_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (mr_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      if (timer > mv_marth_arr_len("THROWNFOXFORWARD", "offset")) {
        timer = mv_marth_arr_len("THROWNFOXFORWARD", "offset") - 1;
      }
      const double grabbedBy = pl->phys.grabbedBy;
      if (grabbedBy == -1) {
      ...
      Vec2D off;
      if (!mv_marth_pair("THROWNFOXFORWARD", "offset", timer - 1, &off)) {
...
const MlMoveDef marth_THROWNFOXFORWARD = {"THROWNFOXFORWARD", mr_init, mr_main, mr_interrupt,
                                0};
