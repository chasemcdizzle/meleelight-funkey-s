// JAB2.c <- src/characters/marth/moves/JAB2.js (M2 task 11)
#include "../moves.h"
...
static AsTri mr_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  mv_assign_hitbox_id(S, p, "jab2", 3, 3);
  mr_main(S, p, in, 0);
  return AS_UNDEF;
}
...
static AsTri mr_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (mr_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (pl->timer > 4 && pl->timer < 15) {
      ml_drawVfx_swing("swing", 0, 0, pl->phys.face, p, "JAB2", pl->timer - 5);
    }
...
const MlMoveDef marth_JAB2 = {"JAB2", mr_init, mr_main, mr_interrupt, 0};
