// CLIFFATTACKQUICK.c <- src/characters/marth/moves/CLIFFATTACKQUICK.js
...
static AsTri mr_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (mr_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (pl->timer < 18) {
      Vec2D off;
      if (!mv_marth_pair("CLIFFATTACKQUICK", "offset", pl->timer - 1,
                         &off)) {
      ...
      pl->phys.cVel.x = mv_marth_arr("CLIFFATTACKQUICK",
                                     "setVelocities",
                                     pl->timer - 18) *
                        pl->phys.face;
    }
...
const MlMoveDef marth_CLIFFATTACKQUICK = {"CLIFFATTACKQUICK", mr_init, mr_main, mr_interrupt,
                           0};
