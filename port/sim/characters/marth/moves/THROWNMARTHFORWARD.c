// THROWNMARTHFORWARD.c <- src/characters/marth/moves/THROWNMARTHFORWARD.js (M2 task 11).
...
static AsTri mr_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (mr_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (timer > 0) {
      if (timer > mv_marth_arr_len("THROWNMARTHFORWARD", "offset")) {
        timer = mv_marth_arr_len("THROWNMARTHFORWARD", "offset") - 1;
      }
      const double grabbedBy = pl->phys.grabbedBy;
      if (grabbedBy == -1) {
      ...
      Vec2D off;
      if (!mv_marth_pair("THROWNMARTHFORWARD", "offset", timer - 1, &off)) {
...
const MlMoveDef marth_THROWNMARTHFORWARD = {"THROWNMARTHFORWARD", mr_init, mr_main, mr_interrupt,
                                0};
