// CLIFFJUMPQUICK.c <- src/characters/marth/moves/CLIFFJUMPQUICK.js
...
static AsTri mr_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (mr_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (pl->timer < 12) {
      Vec2D off;
      if (!mv_marth_pair("CLIFFJUMPQUICK", "offset", pl->timer - 1,
                         &off)) {
...
const MlMoveDef marth_CLIFFJUMPQUICK = {"CLIFFJUMPQUICK", mr_init, mr_main, mr_interrupt,
                           0};
