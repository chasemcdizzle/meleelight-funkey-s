// THROWDOWN.c <- src/characters/marth/moves/THROWDOWN.js (M2 task 11).
#include "../moves.h"
...
static AsTri mr_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  mv_dispatch(S, MV_CS(S, grabbing), "THROWNMARTHDOWN", "init", grabbing, in,
              0);
  mv_turnOffHitboxes(S, p);
  mv_assign_hitbox_id(S, p, "throwdown", 0, 0);
  as_randomShout(MV_CS(S, p));
  const double frame = mv_frames(MV_CS(S, grabbing), "THROWNMARTHDOWN");
...
const MlMoveDef marth_THROWDOWN = {"THROWDOWN", mr_init, mr_main,
                                   mr_interrupt, 0};
