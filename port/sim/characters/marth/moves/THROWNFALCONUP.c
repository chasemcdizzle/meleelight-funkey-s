// THROWNFALCONUP.c <- src/characters/marth/moves/THROWNFALCONUP.js
// (M2 task 11; the fox translation renamed — marth's file is byte-identical modulo imports, offsets from the mvData marth dump). Unguarded family — see THROWNFALCOUP.c.
#include "../moves.h"
...
static AsTri fx_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  strcpy(pl->actionState, "THROWNFALCONUP");
...
static AsTri fx_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fx_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      Vec2D off;
      if (!mv_marth_pair("THROWNFALCONUP", "offset", pl->timer - 1, &off)) {
        mv_out_of_domain("THROWNFALCONUP: offset index out of range");
      }
...
const MlMoveDef marth_THROWNFALCONUP = {"THROWNFALCONUP", fx_init, fx_main,
                                      fx_interrupt, 0};
