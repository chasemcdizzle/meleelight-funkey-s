// UPSMASH.c <- src/characters/marth/moves/UPSMASH.js (M2 task 11)
#include "../moves.h"
...
static AsTri mr_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  mv_assign_hitbox_id(S, p, "upsmash", 0, 0);
  mv_assign_hitbox_id(S, p, "upsmash", 1, 1);
  mv_assign_hitbox_id(S, p, "upsmash", 2, 2);
  mv_assign_hitbox_id(S, p, "upsmash", 3, 3);
  mr_main(S, p, in, 0);
  return AS_UNDEF;
}
...
static AsTri mr_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (mr_interrupt(S, p, in, 0) != AS_TRUE) {
    ...
    if (pl->timer > 10 && pl->timer < 16) {
      ml_drawVfx_swing("swing", 0, 0, pl->phys.face, p, "UPSMASH", pl->timer - 11);
    }
    if (pl->timer == 13) {
...
const MlMoveDef marth_UPSMASH = {"UPSMASH", mr_init, mr_main, mr_interrupt,
                                 0};
