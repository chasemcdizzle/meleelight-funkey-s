// THROWNFALCOUP.c <- src/characters/marth/moves/THROWNFALCOUP.js (M2 task 11; the fox translation renamed — marth's file is byte-identical modulo imports, offsets from the mvData marth dump)
...
// would THROW there — mv_player / the mv_marth_pair trap mirror that).
#include "../moves.h"
...
static AsTri fx_main(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  if (fx_interrupt(S, p, in, 0) != AS_TRUE) {
    if (pl->timer > 0) {
      Vec2D off;
      if (!mv_marth_pair("THROWNFALCOUP", "offset", pl->timer - 1, &off)) {
...
const MlMoveDef marth_THROWNFALCOUP = {"THROWNFALCOUP", fx_init, fx_main,
                                     fx_interrupt, 0};
