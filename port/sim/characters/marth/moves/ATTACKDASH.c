// ATTACKDASH.c <- src/characters/marth/moves/ATTACKDASH.js (M2 task 11)
// setVelocities comes from the mvData marth dump (rule 15).
#include "../moves.h"
...
static AsTri mr_init(MlSim *S, double p, const MlInputBuffer in[4],
  ...
  mv_assign_hitbox_id(S, p, "dashattack", 0, 0);
  mv_assign_hitbox_id(S, p, "dashattack", 1, 1);
  mv_assign_hitbox_id(S, p, "dashattack", 2, 2);
  mv_assign_hitbox_id(S, p, "dashattack", 3, 3);
  mr_main(S, p, in, 0);
  return AS_UNDEF;
}
...
const MlMoveDef marth_ATTACKDASH = {"ATTACKDASH", mr_init, mr_main,
                                    mr_interrupt, 0};
