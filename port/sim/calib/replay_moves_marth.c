// replay_moves_marth.c — M2 task 11 replay driver: feeds recorded
// moves-marth boundary calls (build/<id>.moves-marth.jsonl, FORMAT.md
// "The moves-marth spec") to the C marth-move translations
// (port/sim/characters/marth/, with the task-7 shared bodies linked as
// the nested tree) and compares canon-v1.1 serializations byte-for-byte.
// Task 10's falcon driver adapted per the per-char recipe.
//
// Record types:
// - mvData (frame 0): loads the executed move-data plane — the task-7
//   chars dump PLUS the marth extension {origin, data}: registers the
//   shared C bodies for all five chars and the marth C bodies for char 0
//   per the measured maps (rule 15) and loads the marth move-object data
//   arrays. Registers the marth module for checkForIASA's char-0 arm
//   (mv_register_char_module — actionStateShortcuts.js:400-401; NOTE the
//   arm is dead-by-construction upstream since only fox/falco/falcon
//   aerials call checkForIASA) and marth's onClank special phases
...
// - mdispatch: FIFO seams — the victim's THROWNMARTH* per-char entries
//   reached from marth THROW* chains (args verified, window rng
//   advanced, players/alias/hq resynced).
// - article: marth has ZERO article imports (the pins freeze the count
...
//   ret + the {alias,hq,players,rng,sbid,snd,vfx} post envelope
//   bit-exactly. Marth extension: post carries "sbid" — the Howl play
//   ids consumed by `player[p].shieldBreakerID =
//   sounds.shieldbreakercharge.play()` (a GLOBAL howler counter:
//   unrecoverable chain state, oracle-fed like the task-5 launch
//   getters). The driver FIFOs the recorded ids, injects them through
//   mv_howl_play_id, and re-emits the CONSUMED list into the compared
//   post — a count/order mismatch bites in the compare, and the value
//   itself also lands in player.shieldBreakerID.
//
// Marshalling is STRICT (prevention rule 7): any shape outside the
// captured domain aborts with exit 3 — never guess.
//
// Usage: replay_moves_marth <capture.jsonl> [--strict] [--max-print N]
//                        [--stop-first]
#include <inttypes.h>
#include <setjmp.h>
#include <stdio.h>
...
// --- the marth data plane (mvData marth.data; rule 15) -------------------------
// (measured: setVelocities incl. UPSPECIAL's pairs, CLIFF*/THROWN*
// offsets, THROWNPUFFBACK.offsetVel, the canGrabLedge bool pairs)
#define MARTH_ARRS 192

typedef enum { MRA_NUM, MRA_PAIR, MRA_BOOL } MarthArrKind;
...
static MarthArr g_marth_arr[MARTH_ARRS];
static int g_marth_arr_count = 0;
static bool g_marth_origin[MV_STATES]; // parallel to g_mv[0].state[]
...
static const MarthArr *marth_arr_find(const char *state, const char *key) {
  for (int i = 0; i < g_marth_arr_count; i++) {
    if (strcmp(g_marth_arr[i].state, state) == 0 &&
        strcmp(g_marth_arr[i].key, key) == 0) {
      return &g_marth_arr[i];
    }
  }
  return 0;
}
...
static void load_mvdata(const CanonVal *dump) {
  ...
  const CanonVal *marth = obj_req(dump, "marth");
  ...
  // --- the marth extension: origin map + data arrays --------------------------
  if (marth->type != CV_OBJ || marth->nkeys != 2) fail("mvData marth shape");
  const CanonVal *mdata = obj_req(marth, "data");
  const CanonVal *morigin = obj_req(marth, "origin");
  if (morigin->type != CV_OBJ || morigin->nkeys != g_mv[0].nStates) {
    fail("mvData marth.origin shape");
  }
  int marthCount = 0;
  for (int i = 0; i < morigin->nkeys; i++) {
    if (strcmp(morigin->keys[i], g_mv[0].state[i]) != 0) {
      fail("marth.origin key order");
    }
    g_marth_origin[i] = cv_bool(morigin->vals[i]);
    if (g_marth_origin[i]) {
      marthCount++;
      if (g_mv[0].shared[i]) fail("state both shared- and marth-origin");
      if (strcmp(g_mv[0].state[i], g_mv[0].name[i]) != 0) {
        fail("marth state key != move name");
      }
    }
  }
  if (marthCount != 75) {
    fail("marth.origin: expected 75 marth-origin states");
  }
  if (mdata->type != CV_OBJ) fail("mvData marth.data shape");
  for (int si = 0; si < mdata->nkeys; si++) {
    const CanonVal *entry = mdata->vals[si];
    if (entry->type != CV_OBJ) fail("marth.data entry shape");
    const int sidx = mv_state_idx(0, mdata->keys[si]);
    if (sidx == -1 || !g_marth_origin[sidx]) {
      fail("marth.data on non-marth state");
    }
    for (int ki = 0; ki < entry->nkeys; ki++) {
      const CanonVal *arr = entry->vals[ki];
      if (arr->type != CV_ARR || arr->count > MV_ARR_CAP) {
        fail("marth.data array shape");
      }
      if (g_marth_arr_count >= MARTH_ARRS) fail("marth.data over cap");
      MarthArr *ma = &g_marth_arr[g_marth_arr_count++];
      copy_name(ma->state, mdata->keys[si], "marth.data state too long");
      copy_name(ma->key, entry->keys[ki], "marth.data key too long");
      ma->count = arr->count;
      if (arr->count == 0) fail("marth.data empty array");
      // element kind is uniform per array (measured: scalar rows, [x,y]
      // pairs, or the canGrabLedge booleans)
      if (arr->items[0]->type == CV_NUM) {
        ma->kind = MRA_NUM;
        for (int i = 0; i < arr->count; i++) {
          ma->nums[i] = cv_num(arr->items[i]);
        }
      } else if (arr->items[0]->type == CV_BOOL) {
        ma->kind = MRA_BOOL;
        for (int i = 0; i < arr->count; i++) {
          ma->bools[i] = cv_bool(arr->items[i]);
        }
        ...
        for (int i = 0; i < arr->count; i++) {
          ...
          if (pair->type != CV_ARR || pair->count != 2) {
            fail("marth.data pair shape");
          }
          ma->pairs[i].x = cv_num(pair->items[0]);
          ma->pairs[i].y = cv_num(pair->items[1]);
        }
      } else {
        fail("marth.data element kind");
      }
    }
  }

  // register the shared C bodies for all chars + the marth bodies on
  // char 0, the marth module for checkForIASA's char-0 arm, and the
  // onClank special phases
  for (int c = 0; c < ML_CHARS; c++) {
    ...
    for (int i = 0; i < g_mv[c].nStates; i++) {
      ...
      } else if (c == 0 && g_marth_origin[i]) {
        def = marth_move_def(g_mv[c].state[i]);
        if (def == 0) fail("marthOrigin state has no C body");
      } else {
        continue;
      }
...
// --- the marth data seams (marth moves.h) ----------------------------------------
double mv_marth_arr(const char *state, const char *key, double idx) {
  const MarthArr *ma = marth_arr_find(state, key);
  if (ma == 0 || ma->kind != MRA_NUM) mv_out_of_domain("mv_marth_arr lookup");
  const int i = (int)idx;
  if (idx != (double)i || i < 0 || i >= ma->count) {
...
bool mv_marth_pair(const char *state, const char *key, double idx,
                   Vec2D *out) {
  const MarthArr *ma = marth_arr_find(state, key);
  if (ma == 0 || ma->kind != MRA_PAIR) {
    mv_out_of_domain("mv_marth_pair lookup");
  }
  const int i = (int)idx;
  if (idx != (double)i || i < 0 || i >= ma->count) return false;
  *out = ma->pairs[i];
  return true;
}
...
double mv_marth_arr_len(const char *state, const char *key) {
  const MarthArr *ma = marth_arr_find(state, key);
  if (ma == 0) mv_out_of_domain("mv_marth_arr_len lookup");
  return (double)ma->count;
}

// --- the Howl play-id oracle seam (marth moves.h) ---------------------------------
#define SBID_CAP 8
static double g_sbid[SBID_CAP];
...
int main(int argc, char **argv) {
  ...
  if (!path) {
    fprintf(stderr, "usage: replay_moves_marth <capture.jsonl> [--strict] "
  ...
  while ((n = getline(&line, &linecap, f)) > 0) {
    ...
    *tab1 = 0; *tab2 = 0; *tab3 = 0;
    ...
    if (strcmp(fn, "article") == 0) {
      // marth has ZERO article imports (pinned zero) — a record here is
    ...
    // the recorded Howl play ids for this record (post "sbid"): peek the
    // post envelope BEFORE the args parse (one arena, no dangling refs).
    g_sbid_count = 0;
    g_sbid_used = 0;
    {
      canon_arena_reset();
      const char *perr = 0;
      const CanonVal *post = canon_parse(post_s, &perr);
      if (!post) {
        fprintf(stderr, "PARSE FAIL %s:%ld post: %s\n", path, g_lineno, perr);
        return 3;
      }
      const CanonVal *sb = obj_req(post, "sbid");
      if (sb->type != CV_ARR || sb->count > SBID_CAP) fail("sbid shape");
      for (int i = 0; i < sb->count; i++) {
        g_sbid[g_sbid_count++] = cv_num(sb->items[i]);
      }
    }
    ...
    if (slotArr->type != CV_ARR || slotArr->count != 1) {
      fail("[slot] shape (marth phases carry no extras)");
    }
  ...
  printf("MOVES-MARTH REPLAY RAN %ld records, %ld divergences", replayed,
         g_divergences);
