// replay_moves_fox.c — M2 task 8 replay driver: feeds recorded moves-fox
// boundary calls (build/<id>.moves-fox.jsonl, FORMAT.md "The moves-fox
// spec") to the C fox-move translations (port/sim/characters/fox/, with
// the task-7 shared bodies linked as the nested tree) and compares
// canon-v1.1 serializations byte-for-byte.
//
// Record types:
// - mvData (frame 0): loads the executed move-data plane — the task-7
//   chars dump (shared registration + data seams) PLUS the fox extension
//   {origin, data}: registers the shared C bodies for all five chars and
//   the fox C bodies for char 2 per the measured maps (rule 15), loads
//   the fox move-object data arrays (setVelocities*/offset/...), and
//   registers the fox module index for mv_checkForIASA's FOXMOVES arm.
// - rngBoot / Math.random: the chained seeded mulberry32; frame-0 move
//   records replay on the SEPARATE sweep mulberry32 (0x0badf00d).
// - mdispatch: FIFO seams — the victim's THROWNFOX* per-char entries
//   reached from fox THROW* chains (args verified, window rng advanced,
...
//   hq is an OPAQUE canon string; fox THROW* pushes append row canon via
//   mv_hq_push6 (the value-level model of hitQueue.push).
//
// Marshalling is STRICT (prevention rule 7): any shape outside the
// captured domain aborts with exit 3 — never guess.
//
// Usage: replay_moves_fox <capture.jsonl> [--strict] [--max-print N]
//                         [--stop-first]
#include <inttypes.h>
#include <setjmp.h>
#include <stdio.h>
...
// --- the fox data plane (mvData fox.data; rule 15) ------------------------------
#define FOX_ARRS 96

typedef enum { FA_NUM, FA_PAIR, FA_BOOL } FoxArrKind;
...
static FoxArr g_fox_arr[FOX_ARRS];
static int g_fox_arr_count = 0;
static bool g_fox_origin[MV_STATES]; // parallel to g_mv[2].state[]
...
static const FoxArr *fox_arr_find(const char *state, const char *key) {
  for (int i = 0; i < g_fox_arr_count; i++) {
    if (strcmp(g_fox_arr[i].state, state) == 0 &&
        strcmp(g_fox_arr[i].key, key) == 0) {
      return &g_fox_arr[i];
    }
  }
  return 0;
...
static void load_mvdata(const CanonVal *dump) {
  ...
  const CanonVal *fox = obj_req(dump, "fox");
  ...
  // --- the fox extension: origin map + data arrays ---------------------------
  if (fox->type != CV_OBJ || fox->nkeys != 2) fail("mvData fox shape");
  const CanonVal *fdata = obj_req(fox, "data");
  const CanonVal *forigin = obj_req(fox, "origin");
  if (forigin->type != CV_OBJ || forigin->nkeys != g_mv[2].nStates) {
    fail("mvData fox.origin shape");
  }
  int foxCount = 0;
  for (int i = 0; i < forigin->nkeys; i++) {
    if (strcmp(forigin->keys[i], g_mv[2].state[i]) != 0) {
      fail("fox.origin key order");
    }
    g_fox_origin[i] = cv_bool(forigin->vals[i]);
    if (g_fox_origin[i]) {
      foxCount++;
      if (g_mv[2].shared[i]) fail("state both shared- and fox-origin");
      if (strcmp(g_mv[2].state[i], g_mv[2].name[i]) != 0) {
        fail("fox state key != move name");
      }
    }
  }
  if (foxCount != 61) fail("fox.origin: expected 61 fox-origin states");
  if (fdata->type != CV_OBJ) fail("mvData fox.data shape");
  for (int si = 0; si < fdata->nkeys; si++) {
    const CanonVal *entry = fdata->vals[si];
    if (entry->type != CV_OBJ) fail("fox.data entry shape");
    const int sidx = mv_state_idx(2, fdata->keys[si]);
    if (sidx == -1 || !g_fox_origin[sidx]) fail("fox.data on non-fox state");
    for (int ki = 0; ki < entry->nkeys; ki++) {
      const CanonVal *arr = entry->vals[ki];
      if (arr->type != CV_ARR || arr->count > MV_ARR_CAP) {
        fail("fox.data array shape");
      }
      if (g_fox_arr_count >= FOX_ARRS) fail("fox.data over cap");
      FoxArr *fa = &g_fox_arr[g_fox_arr_count++];
      copy_name(fa->state, fdata->keys[si], "fox.data state too long");
      copy_name(fa->key, entry->keys[ki], "fox.data key too long");
      fa->count = arr->count;
      if (arr->count == 0) fail("fox.data empty array");
      ...
      if (arr->items[0]->type == CV_NUM) {
        ...
        for (int i = 0; i < arr->count; i++) {
          ...
          if (pair->type != CV_ARR || pair->count != 2) {
            fail("fox.data pair shape");
          }
          fa->pairs[i].x = cv_num(pair->items[0]);
          fa->pairs[i].y = cv_num(pair->items[1]);
        }
      } else {
        fail("fox.data element kind");
      }
    }
  }

  // register the shared C bodies for all chars + the fox bodies on char 2
  for (int c = 0; c < ML_CHARS; c++) {
    ...
    for (int i = 0; i < g_mv[c].nStates; i++) {
      ...
      } else if (c == 2 && g_fox_origin[i]) {
        def = fox_move_def(g_mv[c].state[i]);
        if (def == 0) fail("foxOrigin state has no C body");
      } else {
        continue;
      }
    ...
    const int want = c == 1 ? 76 : (c == 2 ? 79 + 61 : 79);
...
// --- the fox data seams (fox moves.h) --------------------------------------------
double mv_fox_arr(const char *state, const char *key, double idx) {
  const FoxArr *fa = fox_arr_find(state, key);
  if (fa == 0 || fa->kind != FA_NUM) mv_out_of_domain("mv_fox_arr lookup");
  const int i = (int)idx;
...
bool mv_fox_pair(const char *state, const char *key, double idx, Vec2D *out) {
  const FoxArr *fa = fox_arr_find(state, key);
  if (fa == 0 || fa->kind != FA_PAIR) mv_out_of_domain("mv_fox_pair lookup");
  const int i = (int)idx;
...
double mv_fox_arr_len(const char *state, const char *key) {
  const FoxArr *fa = fox_arr_find(state, key);
  if (fa == 0) mv_out_of_domain("mv_fox_arr_len lookup");
...
int main(int argc, char **argv) {
  ...
  if (!path) {
    fprintf(stderr, "usage: replay_moves_fox <capture.jsonl> [--strict] "
  ...
  while ((n = getline(&line, &linecap, f)) > 0) {
    ...
    *tab1 = 0; *tab2 = 0; *tab3 = 0;
    ...
    if (slotArr->type != CV_ARR || slotArr->count != 1) {
      fail("[slot] shape (fox phases carry no extras)");
    }
  ...
  printf("MOVES-FOX REPLAY RAN %ld records, %ld divergences", replayed,
         g_divergences);
