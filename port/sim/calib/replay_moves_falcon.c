// replay_moves_falcon.c — M2 task 10 replay driver: feeds recorded
// moves-falcon boundary calls (build/<id>.moves-falcon.jsonl, FORMAT.md
// "The moves-falcon spec") to the C falcon-move translations
// (port/sim/characters/falcon/, with the task-7 shared bodies linked as
// the nested tree) and compares canon-v1.1 serializations byte-for-byte.
// Task 9's falco driver adapted per the per-char recipe.
//
// Record types:
// - mvData (frame 0): loads the executed move-data plane — the task-7
//   chars dump PLUS the falcon extension {origin, data}: registers the
//   shared C bodies for all five chars and the falcon C bodies for char 4
//   per the measured maps (rule 15) and loads the falcon move-object data
//   arrays. NO char-module registration: upstream's checkForIASA has no
//   characterSelections==4 branch (a falcon IASA aerial payload
//   dispatches nothing — the falco precedent).
// - rngBoot / Math.random: the chained seeded mulberry32; frame-0 move
//   records replay on the SEPARATE sweep mulberry32 (0x0badf00d).
// - mdispatch: FIFO seams — the victim's THROWNFALCON* per-char entries
//   reached from falcon THROW* chains (args verified, window rng
//   advanced, players/alias/hq resynced).
// - article: falcon has NO article call sites (dead imports upstream —
//   the pins freeze the count at ZERO); any record still lands in the
//   FIFO and fails as unconsumed (the measured-claim tripwire).
...
//   Falcon extension: phase may be "onPlayerHit" (1-arg) or
//   "onWallCollide" ([slot, wallFace, wallNum] — extras marshalled as
//   DX_STR + DX_NUM). hq is an OPAQUE canon string; falcon THROW*/
//   UPSPECIALCATCH pushes append row canon via mv_hq_push6.
//
// Marshalling is STRICT (prevention rule 7): any shape outside the
// captured domain aborts with exit 3 — never guess.
//
// Usage: replay_moves_falcon <capture.jsonl> [--strict] [--max-print N]
//                         [--stop-first]
#include <inttypes.h>
#include <setjmp.h>
#include <stdio.h>
...
// --- the falcon data plane (mvData falcon.data; rule 15) -------------------------
// (measured: 85 arrays over 50 falcon states, max element count 64)
#define FALCON_ARRS 192

typedef enum { FCA_NUM, FCA_PAIR, FCA_BOOL } FalconArrKind;
...
static FalconArr g_falcon_arr[FALCON_ARRS];
static int g_falcon_arr_count = 0;
static bool g_falcon_origin[MV_STATES]; // parallel to g_mv[4].state[]
...
static const FalconArr *falcon_arr_find(const char *state, const char *key) {
  for (int i = 0; i < g_falcon_arr_count; i++) {
    if (strcmp(g_falcon_arr[i].state, state) == 0 &&
        strcmp(g_falcon_arr[i].key, key) == 0) {
      return &g_falcon_arr[i];
    }
  }
  return 0;
}
...
static void load_mvdata(const CanonVal *dump) {
  ...
  const CanonVal *falcon = obj_req(dump, "falcon");
  ...
  // --- the falcon extension: origin map + data arrays --------------------------
  if (falcon->type != CV_OBJ || falcon->nkeys != 2) fail("mvData falcon shape");
  const CanonVal *fdata = obj_req(falcon, "data");
  const CanonVal *forigin = obj_req(falcon, "origin");
  if (forigin->type != CV_OBJ || forigin->nkeys != g_mv[4].nStates) {
    fail("mvData falcon.origin shape");
  }
  int falconCount = 0;
  for (int i = 0; i < forigin->nkeys; i++) {
    if (strcmp(forigin->keys[i], g_mv[4].state[i]) != 0) {
      fail("falcon.origin key order");
    }
    g_falcon_origin[i] = cv_bool(forigin->vals[i]);
    if (g_falcon_origin[i]) {
      falconCount++;
      if (g_mv[4].shared[i]) fail("state both shared- and falcon-origin");
      if (strcmp(g_mv[4].state[i], g_mv[4].name[i]) != 0) {
        fail("falcon state key != move name");
      }
    }
  }
  if (falconCount != 67) {
    fail("falcon.origin: expected 67 falcon-origin states");
  }
  if (fdata->type != CV_OBJ) fail("mvData falcon.data shape");
  for (int si = 0; si < fdata->nkeys; si++) {
    const CanonVal *entry = fdata->vals[si];
    if (entry->type != CV_OBJ) fail("falcon.data entry shape");
    const int sidx = mv_state_idx(4, fdata->keys[si]);
    if (sidx == -1 || !g_falcon_origin[sidx]) {
      fail("falcon.data on non-falcon state");
    }
    for (int ki = 0; ki < entry->nkeys; ki++) {
      const CanonVal *arr = entry->vals[ki];
      if (arr->type != CV_ARR || arr->count > MV_ARR_CAP) {
        fail("falcon.data array shape");
      }
      if (g_falcon_arr_count >= FALCON_ARRS) fail("falcon.data over cap");
      FalconArr *fa = &g_falcon_arr[g_falcon_arr_count++];
      copy_name(fa->state, fdata->keys[si], "falcon.data state too long");
      copy_name(fa->key, entry->keys[ki], "falcon.data key too long");
      fa->count = arr->count;
      if (arr->count == 0) fail("falcon.data empty array");
      ...
      if (arr->items[0]->type == CV_NUM) {
        ...
        for (int i = 0; i < arr->count; i++) {
          ...
          if (pair->type != CV_ARR || pair->count != 2) {
            fail("falcon.data pair shape");
          }
          fa->pairs[i].x = cv_num(pair->items[0]);
          fa->pairs[i].y = cv_num(pair->items[1]);
        }
      } else {
        fail("falcon.data element kind");
      }
    }
  }

  // register the shared C bodies for all chars + the falcon bodies on
  // char 4 (no char-module registration — checkForIASA has no falcon arm)
  for (int c = 0; c < ML_CHARS; c++) {
    ...
    for (int i = 0; i < g_mv[c].nStates; i++) {
      ...
      } else if (c == 4 && g_falcon_origin[i]) {
        def = falcon_move_def(g_mv[c].state[i]);
        if (def == 0) fail("falconOrigin state has no C body");
      } else {
        continue;
      }
    ...
    const int want = c == 1 ? 76 : (c == 4 ? 79 + 67 : 79);
    if (count != want) fail("registration count");
    as_setupActionStates(c, entries[c], count);
  }
  // falcon's non-phase dispatch surfaces (onPlayerHit/onWallCollide)
  mv_register_special_phases(falcon_special_phase);
  g_mv_loaded = true;
}
...
// --- the falcon data seams (falcon moves.h) ----------------------------------------
double mv_falcon_arr(const char *state, const char *key, double idx) {
  const FalconArr *fa = falcon_arr_find(state, key);
  if (fa == 0 || fa->kind != FCA_NUM) mv_out_of_domain("mv_falcon_arr lookup");
  const int i = (int)idx;
...
bool mv_falcon_pair(const char *state, const char *key, double idx,
                    Vec2D *out) {
  const FalconArr *fa = falcon_arr_find(state, key);
  if (fa == 0 || fa->kind != FCA_PAIR) {
    mv_out_of_domain("mv_falcon_pair lookup");
  }
  const int i = (int)idx;
...
double mv_falcon_arr_len(const char *state, const char *key) {
  const FalconArr *fa = falcon_arr_find(state, key);
  if (fa == 0) mv_out_of_domain("mv_falcon_arr_len lookup");
...
// carrier (falcon THROW*/UPSPECIALCATCH rows).
...
int main(int argc, char **argv) {
  ...
  if (!path) {
    fprintf(stderr, "usage: replay_moves_falcon <capture.jsonl> [--strict] "
  ...
  while ((n = getline(&line, &linecap, f)) > 0) {
    ...
    *tab1 = 0; *tab2 = 0; *tab3 = 0;
    ...
    if (strcmp(fn, "article") == 0) {
      // falcon has NO article call sites (pinned zero) — a record here is
    ...
    if (strcmp(phase, "onWallCollide") == 0) {
      ...
      fail("[slot] shape (falcon phases carry no extras)");
    }
  ...
  printf("MOVES-FALCON REPLAY RAN %ld records, %ld divergences", replayed,
         g_divergences);
