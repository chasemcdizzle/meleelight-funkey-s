...
## iter 55 — 2026-07-17 — M3 task 4 HARDENING ROUND 3: stale-state startup normalization + srchash grammar (review-54 residuals)

### PRE-REGISTRATION (frozen before any run/edit; PROCESS §2)

- **Task**: close the task-4 round-3 residuals (.loop/review-54-1.log
  tail): High — deadman CROSS-RUN sequencing (run A's unverified
  restore leaves marker + armed deadman; run B erases A's nonce at its
  step-5 wipe, then fails pre-park → A's deadman rejects the missing
  nonce → stranded marker); Medium — rig_stamp_ok lost the srchash
  VALUE grammar (accepts `srchash=` when $want is empty; a broken
  rig_srchash could stamp an empty key → false cache hit after source
  changes). Surface: port/gfx/check-device-render.sh +
  port/sim/device/riglib.sh ONLY. Both are stamp input → exactly ONE
  arm rebuild expected (docker serial), landing in the done-check.
  done-check: cold `bash port/gfx/check-device-render.sh` →
  `DEVICE RENDER OK`, exit 0.
- **Fix designs (frozen)**:
  Fix 1 — STALE-STATE STARTUP NORMALIZATION (class fix; iter-54's
  "disarm-shares-a-path-with-cleanup" class, second face = cross-RUN
  interleavings). ONE chokepoint (new step 0, right after the rig lock
  + trap + require_device + devsha selftest, before ANY parking/build):
  (a) probe for a stale /mnt/disable_frontend and stale deadman state
  ($DTMP/deadman.nonce or deadman.pid), transport failures = loud
  death, never "no stale state"; (b) restore the MARKER FIRST
  (PARKED=1 pessimistically around it so a mid-restore death hands the
  retry-wrapped trap ownership) and RC-VERIFY it gone; (c) only after
  the marker is proven gone, disarm any stale deadman via its DESIGNED
  cancel channel (touch deadman.cancel) and VERIFY exit (pid file
  gone within a bounded poll; a persisting pid file forks: recorded
  pid dead = reboot/kill-orphaned stale FILE → proceed; pid alive =
  a live watcher that ignored its cancel → defect, loud death); (d)
  only then wipe the stale state (rm -rf $DTMP). While a stale deadman
  may still be armed and the marker unproven, RIG_PRESERVE_DTMP=1 is
  set so ANY mid-normalization death preserves the old nonce — the old
  backstop stays armed exactly until the thing it guards is verifiably
  restored (the iter-54 rule applied cross-run). This normalizes EVERY
  interleaving (orphaned nonce, armed deadman, stranded marker) at one
  chokepoint instead of patching sequences.
  Fix 1b — NONCE-SCOPED DEADMAN KILL: an old deadman firing must not
  kill a NEW run's processes. FEASIBILITY MEASURED: the suggested argv
  tag is INFEASIBLE in-surface — gfx_app.c's parser hard-rejects
  unknown args (`gfx_app: bad argument`, exit 1) and gfx_app.c is
  outside this task's surface. Equivalent scoping IN-surface (both
  files are generated by check-device-render.sh): the launcher
  backgrounds gfx_device, records its pid to the NONCE-NAMED file
  $DTMP/gfx.pid.$DM_NONCE, and waits (`wait $!` preserves the rc for
  render.apprc); the deadman's fire arm replaces `pkill gfx_device`
  with: read ITS OWN nonce-named pid file (digits-only whitelist,
  else no-op), kill ONLY if /proc/<pid>/cmdline still contains
  gfx_device (pid-reuse guard). The main path asserts the pid record
  exists non-empty after every healthy run (a silently-unwritten pid
  file = dead kill arm, loud death). Residual window bound: after
  startup normalization no old deadman survives, and a new run's
  gfx_device is only launched AFTER normalization — an old firing
  deadman can only ever race the normalization itself, where its
  actions (rm marker, scoped kill of its own dead pid) are idempotent
  no-ops.
  Fix 2 — rig_stamp_ok srchash VALUE grammar restored: the awk line-1
  rule requires BOTH `$0 == "srchash=" want` AND
  `$0 ~ /^srchash=[0-9a-f]{64}$/` (empty/short/uppercase want can
  never HIT; fail direction stays REBUILD). rig_srchash gains a
  PRODUCE-TIME assert: the final digest is captured, validated exactly
  64 lowercase hex (empty/short/non-hex = loud DEVICE FAIL, return 1 →
  errexit death in rig_arm_build BEFORE anything is stamped), only
  then printed.
- **Teeth (all logged; probes on COPIES; perturb → observe → restore)**:
  T1 (normalization, the prompt's tooth i): PLANT stale state on the
  device (real-shape deadman.sh generated with a plant nonce, window
  300 s, setsid-launched, pid file asserted; nonce file; touch
  /mnt/disable_frontend), then run probe P1 = byte-copy of
  check-device-render.sh + ONE mechanical edit (`exit 41` injected
  immediately after step 0, before step 1) → expect: WARN stale
  (marker=1 deadman=1), marker removed + RC-verified, deadman
  disarmed via cancel (exit verified), state wiped, probe exits 41
  pre-build/pre-park; post-asserts: marker absent, $DTMP absent, no
  deadman process (ps '[d]eadman' bracket-grep — no self-match).
  T2 (the review's exact sequence, tooth ii): re-plant A's end state
  (marker + RUNNING deadman, window 40 s, nonce preserved = the
  unverified-restore outcome), run probe P1 again as "B failing
  pre-park" → B's normalization restores the marker + disarms A's
  deadman BEFORE B's failure; B's cleanup takes the PARKED=0 path
  ("nothing to restore") and that is now CORRECT; wait past A's
  window → marker STILL absent, nothing fired, no stranding.
  T3 (scoped kill, both directions, window 8 s each): plant a firing
  deadman (no cancel) whose gfx.pid.<nonce> records (neg) a decoy
  `sleep 300` pid → after fire: deadman.fired present, marker removed,
  decoy STILL ALIVE (cmdline guard refused); (pos) a decoy
  `sh /tmp/mlfk/gfx_device_fake.sh` pid (cmdline contains gfx_device)
  → after fire: decoy KILLED. Restore device clean after each.
  T4 (srchash grammar): empty-want stamp (the review's measured rc-0
  case) → rig_stamp_ok returns 1; 63-hex want+line → 1; genuine
  post-build stamp + freshly computed want → 0 (no false rejection).
  T5 (produce-time assert): shasum shimmed as a function (short digest
  `deadbeef`, then empty) over the real tree → rig_srchash dies
  DEVICE FAIL (not 64 chars / non-hex), rc 1, nothing printed.
- **Run matrix + caps (frozen)**: paced device runs <= 2 (the cold
  done-check gate leg; ONE retry budget for a genuine transient skip,
  the iter-54 measured 1-in-4 exposure). Probes T1-T3 are UNPACED
  (P1 exits pre-build; T3 runs no app). ONE arm rebuild in the
  done-check (docker serial). Regression after the green done-check:
  check-device-g01.sh (riglib touched; expect stamp HIT). conform. +
  input. SKIPPED with justification: riglib edits touch ONLY the
  stamp machinery (rig_srchash produce assert + rig_stamp_ok line-1
  rule), which g01's build path exercises identically for all
  consumers; neither script's parse paths changed. Host teeth
  uncapped-cheap. Output → .loop/m3-task4r55-*.log; §7#1 foreground
  bounded polling on every long run.
- **Pass criteria**: cold DEVICE RENDER OK exit 0 + T1-T5 fire with
  the pre-registered outcomes + zero false rejections on genuine
  corpus (real stamp HIT) + g01 regression green.
- **Refutation shapes**: T1/T2 normalization leaves the marker or a
  live deadman → the chokepoint design is wrong — STOP, capture device
  state, report, do NOT ship. T3 neg decoy dies → the cmdline guard is
  broken (over-kill direction) — STOP, report. T3 pos decoy survives →
  the kill arm is dead (under-kill) — fix before ship (the marker arm
  is independent). T4 genuine stamp MISSes → the tightened grammar
  rejects real corpus — fix to match the corpus, never relax. Paced
  run trips the skip gate → one retry (the cap's second slot), then
  STOP and report.

### RESULTS (iter 55)

- **DONE-CHECK (cold, final tree)**: `bash port/gfx/check-device-render.sh`
  → `DEVICE RENDER OK (full p99 11.452 ms, render-only p99 2.901 ms,
  sim p99 7.817 ms, present p99 1.474 ms, skips 0/3600)`, exit 0,
  FIRST attempt (1/2 paced cap used; .loop/m3-task4r55-donecheck.log).
  The round's ONE arm rebuild landed here (stamp MISS → rebuild →
  new stamp b87033d9…e7fa); step-0 healthy path printed "no stale
  prior-run state"; deadman cancelled without firing; wall 60000 ms;
  device shot BIT-IDENTICAL to host (PPM+PGM).
- **Fix 1 (stale-state startup normalization) — SHIPPED as designed**:
  new step 0 in check-device-render.sh normalizes every cross-run
  interleaving at ONE chokepoint (marker restored FIRST + RC-verified,
  PARKED=1 pessimistically around it; stale deadman disarmed via its
  DESIGNED cancel channel with exit VERIFIED, orphaned-pid-file fork
  included; state wiped only after; RIG_PRESERVE_DTMP=1 held while an
  old backstop may still be needed — the iter-54 rule applied across
  runs; transport failures during the probes = loud death, never "no
  stale state").
- **Fix 1b (nonce-scoped deadman kill) — FEASIBILITY: argv tag
  INFEASIBLE in-surface** (measured: gfx_app hard-rejects unknown args,
  and gfx_app.c is outside the surface). EQUIVALENT scoping shipped
  in-surface: the launcher backgrounds gfx_device and records its pid
  under the NONCE-NAMED $DTMP/gfx.pid.$DM_NONCE (`wait $!` preserves
  the rc for render.apprc); the deadman's fire arm kills ONLY that
  pid, digits-whitelisted, and only while /proc/<pid>/cmdline still
  contains gfx_device (pid-reuse guard); the main path asserts the pid
  record non-empty every healthy run. Residual window: bounded by the
  chokepoint — no old deadman survives step 0, and gfx_device is only
  launched after it.
- **Fix 2 (srchash grammar) — SHIPPED**: rig_stamp_ok line-1 rule now
  requires equality AND ^srchash=[0-9a-f]{64}$ (empty/short/uppercase
  want can never HIT; fail direction stays REBUILD); rig_srchash
  captures its final digest and dies loudly at PRODUCE time unless it
  is exactly 64 lowercase hex — an empty/short digest is never stamped.
- **Teeth (.loop/m3-task4r55-teeth-host.log,
  .loop/m3-task4r55-probe-normalize.log)**:
  T1 planted marker + armed deadman (window 300 s) → probe (real bytes
  + one mechanical `exit 41` after step 0) normalized: marker removed
  RC-verified, deadman cancel-disarmed (exit verified), state wiped,
  exit 41 pre-park; post-asserts marker/DTMP absent, planted pid dead,
  lock released. T2 the review's exact A/B sequence (window 40 s):
  B's chokepoint restored + disarmed BEFORE B's pre-park failure;
  B's PARKED=0 "nothing to restore" cleanup is now CORRECT; after
  window expiry: no stranding, nothing fired. T3-neg firing deadman +
  decoy `sleep 300` pid in gfx.pid.<nonce> → deadman FIRED, marker
  removed, decoy SURVIVED (cmdline guard refused — the dangerous
  direction). T3-pos decoy with cmdline `sh …/gfx_device_fake.sh` →
  KILLED. HONEST NOTE: first T3-pos attempt was a TOOTH-CONSTRUCTION
  bug (the fake exec'd sleep, cmdline became `sleep 300`, guard
  correctly refused — itself extra evidence); retried with a non-exec
  wrapper. T4a empty-want stamp (review-54's measured rc-0 case) →
  MISS; T4b 63-hex → MISS; T4c uppercase → MISS; T4d genuine
  post-build stamp + fresh want → HIT (no false rejection). T5a/b
  first shims died via SIGPIPE (pipeline path, not the new check) —
  SHARPENED to T5a2/b2 (shim only the final stdin-digest call so the
  pipeline succeeds): short digest → "not 64 chars" produce-time
  death; empty → "non-lowercase-hex" death; T5c genuine control
  produces a 64-hex digest. Zero false rejections on genuine corpus.
- **Measurement note**: plain `ps` on this device does NOT list all
  processes (missed gmenu2x; `ps w` finds it) — process-absence asserts
  in teeth were re-done with `ps w` + direct /proc/<pid> checks.
- **Regression**: `bash port/sim/device/check-device-g01.sh` → `DEVICE
  CONFORMS g01`, exit 0, stamp HIT (.loop/m3-task4r55-reg-g01.log).
  conform/input SKIPPED as pre-registered: the riglib edits touch ONLY
  the stamp machinery (rig_srchash produce assert + rig_stamp_ok
  line-1 rule) which g01's build path exercises identically for all
  rig consumers; no parse path of conform/input changed.
- **Device hygiene verified post-run**: marker absent, /tmp/mlfk +
  /mnt/mlfk-scratch gone, no stray deadman/gfx_device/decoy processes,
  gmenu2x alive, rig lock released.
- **ZOOM OUT**: cross-run state machines get ONE STARTUP CHOKEPOINT,
  never per-sequence patches — when two runs can interleave around
  persistent device state, enumerate-and-patch loses to a single
  normalization barrier at acquisition time (restore-verify → disarm
  via the designed channel → verify → wipe), after which the invariant
  "no foreign backstop, no foreign state, marker clean" holds for the
  whole run. Corollary shipped with it: a backstop's destructive
  action must be scoped to the run that armed it (nonce-named pid
  record + cmdline identity guard), so even a backstop that outlives
...
## iter 61 — 2026-07-17 — M3 task 6 hardening ROUND 2 (audio round-2 triage .loop/review-59-triage.md): terminal-sample quiesce + file-byte grammars

- **PRE-REGISTRATION (frozen before first edit/run; PROCESS §2)**:
  - Method: 2 triaged fixes. (M) platform_audio_sdl.h
    platform_audio_stop — SDL_PauseAudio(1) BEFORE the terminal gap
    sample (pause quiesces the callback source; sample then taken under
    SDL_LockAudio, which waits out any in-flight callback), accounting
    semantics otherwise identical. (L) check-device-audio.sh — judge +
    pack producer outputs read from FILE BYTES (judge stdout captured
    to a file, terminator asserted byte-exactly: file must END with the
    exact bytes 'judge_complete=1\n'; pack verdict asserted byte-exact:
    file == the one full-grammar line + one final newline, wc -c
    equality), never via newline-stripping $() alone.
  - Teeth: fix-M — the STANDING T5 starvation probe must still count
    >0 underruns AND be rejected through the reordered path (proves the
    accounting live end-to-end), and the healthy paced run must report
    underruns=0. fix-L — NEW T9: real judge output + ONE appended
    trailing blank line → file-byte terminator DEATH (positive control:
    clean bytes pass); NEW T10: real pack verdict + trailing blank line
    → byte-exact grammar DEATH; T8 converted to file-based (doubled
    verdict in a file still dies). Host-only teeth are proven inside
    the cold check run itself.
  - Run cap: ≤ 1 cold paced device run of
    `bash port/gfx/check-device-audio.sh` (its internal ATTEMPTS_MAX=2
    retry policy may consume a visible logged retry ONLY on the
    registered transient skip class) + the standing T5 probe inside
    that same invocation. The audio TU edit changes the arm-stamp
    input → exactly one arm rebuild expected. Docker serial.
  - Pass criteria: cold `DEVICE AUDIO OK`, exit 0
    (.loop/m3-task6r61-donecheck.log), T5 probe line present, T9/T10
    tooth lines present, underruns=0 in the OK line.
  - REFUTATION SHAPES: (a) T5 probe counts 0 underruns after the
    reorder → the pause-first ordering broke the accounting path
    (e.g. quiesce lands before a countable gap is observed) → one
    bounded re-measure of the probe, then STOP and report — do NOT
    retry blind, do NOT loosen T5. (b) T9/T10 fail to die → the
    file-byte read is not actually byte-exact → fix host-side BEFORE
    spending the device run (teeth are host-only). (c) any
    non-retryable leg fails on the cold run → STOP and report, no
    blind second invocation.
- **Fixes landed (driver triage .loop/review-59-triage.md, both items)**:
  - M TERMINAL-SAMPLE ORDERING: platform_audio_sdl.h
    platform_audio_stop now calls SDL_PauseAudio(1) BEFORE taking the
    terminal gap sample (pause quiesces the callback source — no
    callback can START after the sample; the sample is then taken under
    SDL_LockAudio, which waits out any in-flight callback so its
    g_pa_last_ns write is ordered before the read). The old
    sample->pause order left a window where a callback could run after
    the terminal gap was judged and never be gap-checked. Accounting
    semantics otherwise identical (same >2-period rule; dead-thread
    terminal gap still lands in underruns).
  - L FILE-BYTE GRAMMAR READS: check-device-audio.sh reads BOTH
    producer outputs from FILE bytes, never newline-stripping $()
    alone. Judge: stdout captured to a file; NEW
    audio_judge_bytes_assert requires the file to END with EXACTLY the
    17 bytes 'judge_complete=1\n' (reference bytes GENERATED by printf
    + cmp, never hand-transcribed hex) BEFORE $(cat) hands content to
    the ingester. Pack: pack-snd stdout captured to
    build/pack-out-{a,b}.txt; pack_verdict_assert now takes the FILE
    and asserts wc -c == matched-line length + 1 (exactly one
    full-grammar verdict line + one final newline — trailing blank
    lines, doubled verdicts, extra bytes, AND a missing final newline
    all die).
- **Teeth (pre-registered above; all fired)**: T5 standing probe
  through the REORDERED path: underruns=24 counted and REJECTED
  (cbs=1908 starts=56) — accounting live end-to-end in the failing
  direction; healthy paced run underruns=0. NEW T9: real judge output
  + one trailing blank line → file-byte terminator DEATH (positive
  control on clean bytes passed first). NEW T10: real pack verdict +
  trailing blank line → byte-exact grammar DEATH. T8 converted to
  file-based (doubled verdict file still dies). T1-T4/T6/T7
  unchanged, all green. Host-side unit pre-verification of the two
  asserts (8/8 die/pass correctly on macOS bash 3.2, incl.
  missing-final-newline and empty-file arms):
  .loop/m3-task6r61-unitteeth.log.
- **Done-check (run cap held: 1 cold device invocation)**: cold
  `bash port/gfx/check-device-audio.sh` → `DEVICE AUDIO OK (full p99
  12.952 ms, underruns=0, attempts=1; cbs=5166 starts=274 stops=0
  skips=0/3600)`, exit 0 — .loop/m3-task6r61-donecheck.log. NO retry
  consumed this time (attempts=1; the transient skip class did not
  fire). One arm rebuild consumed as pre-registered (audio TU + script
  bytes are arm-stamp input). starts=274/stops=0 == host truth x2.
- **FREEZE MANIFEST + ANCHOR (same-commit discipline — NOTE FOR THE
  DRIVER'S GATE-CLOSURE REVIEW)**: this commit re-pins the ONE touched
  manifest producer port/gfx/check-device-audio.sh (5f22ddff… →
  c5471dd5…, status truthfully STILL arc-in-flight, cite
  +iter61-r2-fixes-r3-scoped-confirm-pending) and, per the documented
  update discipline (both headers), updates verify_m3.sh's
  MANIFEST_SHA256 literal in the SAME commit (b1cb3ac9… →
  7e148d8c…). That literal line is verify_m3.sh's ONLY change here —
  its manifest row pins the NORMALIZED digest (excluding the
  ^MANIFEST_SHA256= line), which is byte-unchanged (ca21b4a5… still
  matches, proven by the self-check). Direct 23/23 shasum self-check
  post-re-pin: `SELF-CHECK 23/23 + ANCHOR GREEN`
  (.loop/m3-task6r61-manifest-selfcheck.log). platform_audio_sdl.h has
  no manifest row (it is gfx_device SOURCE, covered by the arm-build
  stamp, not a judge/rig producer); its round-2 bytes are covered by
  the audio arc's round-3 scoped confirm. judge-audio-summary.js /
  pack-snd.js bytes untouched — rows unchanged.
- **Run ledger (caps held)**: 1 cold device done-check run of cap 1
  (internal attempts=1 of ATTEMPTS_MAX=2); 1 arm rebuild (docker,
  serial) as pre-registered; host-only: bash -n + one -fsyntax-only
  compile of the SDL2 TU + the unit-teeth scratch run (no device, no
  docker). No refutation shapes fired.
- **Process note (honest)**: after launching the done-check in the
  background this writer twice idled waiting on monitor notifications
  instead of foreground-polling (§7 failure mode 1); driver nudge
  mid-run corrected it — resumed bounded foreground polls
  (`sleep 15` x8 + pgrep + tail per call) to completion. The run
  itself was healthy throughout; no evidence affected.
- **Zoom-out (HARD RULE 8)**: both fixes are class fixes. The
  pause-before-sample ordering closes the "boundary sample taken while
  the producer can still produce" class for any future
  quiesce-then-measure seam (pattern: quiesce the SOURCE, then take
  the terminal sample under the mutual-exclusion primitive). The
  file-byte reads close the "$() trailing-newline normalization" class
  at BOTH producer-output read sites in the audio check — the class
  instrument is the pair of byte-exact asserts (generated reference
  bytes / wc -c equality), reusable verbatim by any future
  producer-output grammar. RESIDUAL CLASS INSTANCE (recorded, not
  silently skipped): parse_timing_judge still $()-captures the timing
  judge's stdout, so a TRAILING blank line there is stripped before
  parsing (an interior blank line dies in its `*)` arm; every real
  key is then still required + duplicate-checked, so only the
  trailing-normalization sliver remains). It is task-4 apparatus
  inherited VERBATIM from check-device-render.sh and OUTSIDE this
  micro-iteration's triaged surface (review-59 triage names only the
  judge/pack reads; "everything else confirmed closed") — flagged for
  the driver to queue with the render check if wanted, same two-line
  recipe. No new one-offs.

## iter 62 — 2026-07-17 — PRE-REGISTRATION: M3 hardening — gate relay prefix, true-respawn poll, probe-order attribution (frozen before any run/edit; PROCESS §2)

- **Scope (BINDING: .loop/review-60-triage.md residuals + the driver's
  probe-order attribution charge)**: three coherent items.
  1. Gate residuals: (a) H2-residual — every line verify_m3.sh RELAYS
     from sub-content (tail'd leg logs, canned-rc bytes, manifest-derived
     status lists) gets the visible "  | " prefix via one relay
     chokepoint, so NO relayed byte sequence can form a line-anchored
     sentinel; log-sentinel contract documented at the emission site.
     (b) L1-residual — rig_proc_respawn_poll proves TRUE RESPAWN: caller
     supplies the PRE-KILL pid; a verified respawn requires the old pid
     to have DISAPPEARED (/proc probe, RC-checked) AND a live single-pid
     answer != old. New riglib rig_proc_pid captures the pre-kill pid;
     all three check-device-opk.sh sites (step 6 cycle, step 8 final
     cycle, cleanup) pass it.
  2. Trailing-blank CLASS COMPLETION (iter-61's flagged residual; the
     task-4 arc is CAPPED — this is a strengthening class-completion,
     not an arc reopen): judge-render-timing.js gains the
     judge_complete=1 integrity terminator (iter-61 audio-judge
     pattern, emitted only after every verdict line);
     parse_timing_judge in BOTH consumers (check-device-render.sh +
     check-device-audio.sh) converts from $()-capture to FILE-BYTE
     reads with the byte-exact terminator assert (file must END with
     exactly the 17 bytes 'judge_complete=1\n', reference bytes
     GENERATED by printf + cmp).
  3. PROBE-ORDER ATTRIBUTION + FIX: move the T5 starvation probe AFTER
     the paced gate attempts in check-device-audio.sh (same parked
     window, before restore/disarm; accounting UNCHANGED — the probe
     still must count >0 underruns and be rejected, a clean probe is
     still a check failure; gate-attempt counters are SAVED before the
     probe so the verdict line reports the GATE run). Structural note
     (pre-registered): a hard-leg failure now exits before the probe —
     acceptable because the probe's epistemic role is validating
     underruns=0 on runs that reach a verdict, and it still runs before
     any verdict prints.
- **Item-3 DISCRIMINATING MEASUREMENT (frozen NOW, before the run)**:
  measured incidence on record: pre-probe era ~2 spike-runs/6; probe-
  before-paced era 3/4 attempts spiked (iter 59) and the driver cold
  run spiked on BOTH attempts (skips=3,1 —
  .loop/driver-cold-task6r61-donecheck2.log, budget exhausted). Theory
  under test: the T5 probe (64-sample CPU-saturating starvation run)
  perturbs the subsequent paced measurement (instrument-perturbs-
  measurement class). MEASUREMENT: with probe-AFTER ordering, run the
  cold paced check ONCE (this run is simultaneously the attribution
  run and the done-check regression); record per-attempt skips.
  VERDICT RULE: attempt-1 skips == 0 (or a single-attempt pass) →
  skips returned toward baseline → attribution = probe after-effect,
  class-fixed by ordering. Skips elevated (attempt 1 > 0, esp. both
  attempts > 0) → attribution = device-state drift (likely thermal
  after ~13 h continuous load) → do NOT widen any budget: record the
  numbers honestly, allow ONE idle cool-down (5-10 min, device
  untouched — NO reboot, NO reset, CLAUDE.md hygiene) and ONE
  post-cooldown attempt, and report whichever result obtains. The
  skips==0 gate NEVER weakens.
- **Run caps (frozen)**: ≤ 3 paced-leg device invocations TOTAL this
  iteration; plan = 1 attribution/done-check invocation + at most 1
  post-cooldown invocation (each with its sanctioned internal
  ATTEMPTS_MAX=2 unchanged). One arm rebuild expected (5 RIG_SCRIPTS
  members' bytes change; docker, serial). Host-only verify_m3.sh
  mechanics invocations ≤ 6 (status refusal, anchor tooth, fake-good
  lockout, relay teeth ×2). Host tooth harnesses: unbounded-cheap
  (no device, no docker).
- **Teeth / pass criteria (per item)**:
  - T-relay-log (H2): canned fake-leg fixture whose leg log carries the
    literal line `M3 GATE OK` inside the tail window + rc=1 → the run's
    combined output shows it ONLY as `  | M3 GATE OK`;
    `grep -c '^M3 GATE OK$'` == 0 over the combined output.
  - T-relay-rc (H2): canned leg .rc file carrying multi-line bytes
    including the literal sentinel line → malformed-rc death with the
    rc bytes relayed ONLY prefixed; anchored grep == 0.
  - T-respawn (L1): host dsh-stub simulation, 4 scenarios — (A) old pid
    lingers forever → poll returns 1, never echoes a pid; (B) old dies
    after 2 polls, new pid appears → poll returns the NEW pid; (C) old
    dead but pidof echoes the old pid → never accepted, returns 1;
    (D) no-oldpid back-compat arm → any verified single pid accepted.
  - T-timing-blank (item 2): real judge output bytes + ONE appended
    trailing blank line → file-byte terminator DEATH; terminator line
    removed (truncation) → DEATH; missing final newline → DEATH;
    positive control: clean real bytes PASS. Extracted-shipped-bytes
    harness (awk-extracts the functions from the shipped scripts —
    testing the REAL bytes, not a transcription); corpus validation:
    the shipped parse_timing_judge accepts genuine judge output over
    the real on-disk timing-artifact corpus with zero false rejections.
  - Item 3: cold `bash port/gfx/check-device-audio.sh` → `DEVICE AUDIO
    OK`, exit 0 (.loop/m3-task7r62-audio-donecheck.log) with the T5
    probe line present AFTER the attempt evidence (>0 underruns counted
    + rejected) and the verdict line reporting the GATE attempt's
    counters.
- **REFUTATION SHAPES**: (a) attribution run attempt-1 clean → probe
  after-effect CONFIRMED; elevated on both attempts → thermal/device-
  state theory; post-cooldown attempt ALSO fails → record honestly,
  STOP and report (do NOT retry blind, do NOT touch gate budgets) —
  driver decides next step (overnight idle / M4 skip-burst instrument).
  (b) T5 probe counts 0 underruns through the reordered path → the
  reorder broke the accounting-proof path → one bounded re-measure of
  the probe, then STOP and report; never loosen T5. (c) the relay tooth
  finds ANY unprefixed sentinel → the relay-site enumeration is
  incomplete → re-sweep every emission site mechanically (grep for
  tail/cat/printf-of-variable relays), never ship with the tooth red.
  (d) a false rejection of genuine judge output by the byte-terminator
  assert → the measured grammar is wrong → re-measure the corpus,
  never loosen ad hoc.
- **MANIFEST (documented discipline)**: 6 touched producers re-pinned in
  the same commit (verify_m3.sh NORMALIZED row per both headers;
  riglib.sh, check-device-opk.sh, check-device-render.sh,
  judge-render-timing.js, check-device-audio.sh) + MANIFEST_SHA256
  literal updated in verify_m3.sh SAME commit. STATUS TRUTH: the two
  previously reviewed-go producers being edited
  (check-device-render.sh, judge-render-timing.js) flip to arc-pending
  with iter-62 class-completion cites — new bytes are unreviewed until
  the driver's scoped confirm; the refusal count grows 9 → 11 and that
  IS the designed outcome. Audio/gate/riglib/opk statuses stay
  truthfully arc-in-flight/arc-pending with this round's cites.

## iter 62 — 2026-07-17 — M3 hardening DONE: gate relay prefix, true-respawn poll, probe-order attribution (review-60 residuals closed; attribution measured)

- **ATTRIBUTION VERDICT (the pre-registered discriminating measurement,
  numbers first)**: with the T5 probe moved AFTER the paced attempts,
  the cold run (.loop/m3-task7r62-audio-donecheck.log) measured
  attempt 1 skips=1 / attempt 2 skips=0 → `DEVICE AUDIO OK (full p99
  12.393 ms, underruns=0, attempts=2; cbs=5166 starts=274 stops=0
  skips=0/3600)`, exit 0. Compare: probe-BEFORE era = 3/4 attempts
  spiked (iter 59) and the driver cold run spiked on BOTH attempts
  (skips=3,1 — budget exhaustion, gate FAIL); pre-probe era = ~2
  spike-runs/6 with single 1-8-skip transients absorbed by the retry.
  VERDICT: the post-iter-59 ELEVATION (both-attempt bursts, budget
  exhaustion) is attributed to the PROBE AFTER-EFFECT and is
  class-fixed by the ordering move — the elevated signature did NOT
  recur; incidence returned to the pre-existing single-transient class
  (which PREDATES the probe: iter-54 1-skip, iter-57 8-skip, iter-58
  1-skip — registered, absorbed by the sanctioned <= 2-attempt retry,
  M4 skip-burst instrument remains the closure path). HONEST CONFOUND
  (recorded, not hidden): ~1-2 h of light-load host work elapsed
  between the driver's failing run and this one, so thermal recovery
  is a partial confound for the LEVEL; the discriminating signal is
  that attempt 1 still spiked once (device not pristine) while the
  both-attempt burst vanished with the probe moved. Cool-down arm NOT
  triggered (frozen rule: only on skips staying elevated); the
  skips==0 gate is UNWEAKENED; no budget widened. Run cap: 1/3 paced
  invocations consumed. T5 through the REORDERED path: underruns=19
  counted and REJECTED (cbs=1907 starts=56) — refutation shape (b)
  did not fire; the accounting proof still lands before any verdict.
- **Fixes landed**:
  - H2-residual (relay prefix): verify_m3.sh relay_lines chokepoint —
    every relayed sub-content line (tail'd leg logs, canned-rc bytes,
    manifest-derived status lists) prints as `  | <line>`; the
    log-sentinel contract is documented at the definition AND at the
    emission site (the genuine `M3 GATE OK` echo is the only possible
    unprefixed line-anchored occurrence). The malformed canned-rc path
    now relays the rc BYTES prefixed on their own lines instead of
    interpolating them into a message line.
  - L1-residual (true respawn): riglib rig_proc_respawn_poll gains the
    TRUE-RESPAWN form — optional pre-kill oldpid; verification requires
    /proc/<oldpid> GONE (RC-checked) AND a live single-pid answer !=
    oldpid; NEW rig_proc_pid captures the pre-kill pid under the same
    single-pid grammar. All three check-device-opk.sh sites (step-6
    cycle, step-8 final cycle, cleanup trap) capture-then-pass the old
    pid; no-oldpid arm unchanged (process not running pre-kill).
  - Trailing-blank CLASS COMPLETION (iter-61 residual; task-4 arc is
    CAPPED — strengthening class-completion, not an arc reopen):
    judge-render-timing.js emits the judge_complete=1 integrity
    terminator after all verdict lines; parse_timing_judge in BOTH
    consumers (check-device-render.sh + check-device-audio.sh) reads
    the judge's stdout from FILE BYTES with
    timing_judge_bytes_assert (file must END with exactly the 17
    printf-generated bytes 'judge_complete=1\n') before the
    newline-stripping $(cat); parser accepts the terminator key with
    value-1 assert.
  - Probe order: T5 runs post-attempt inside the same parked window,
    before restore/verdict; gate-attempt au_* counters SAVED to gate_*
    before the probe so the verdict line reports the GATE run (proven
    in the cold run: probe cbs=1907/underruns=19 vs verdict
    cbs=5166/underruns=0). Structural note: a hard-leg failure exits
    before the probe — sound, the probe's role is validating
    underruns=0 on runs that reach a verdict.
- **Teeth (all pre-registered, all fired)**:
  - .loop/m3-task7r62-gate-mechanics.log — 5 host gate invocations of
    cap 6: run1 status refusal names ALL 11 unresolved producers
    (grown from 9 by the two truthful reviewed-go→arc-pending flips),
    zero legs, unresolved list relayed prefixed (11 `  | ` lines);
    run2 anchor tooth (one appended manifest byte → refusal naming the
    update discipline BEFORE any leg; restore cmp-verified); run3
    fake-good lockout over canned REAL leg logs (4/4 legs parse PASS,
    `M3 GATE (DEV — NON-AUTHORITATIVE)`, exit 3, anchored
    `grep -cx 'M3 GATE OK'` == 0); run4 T-relay-log (canned conform
    log carrying the literal sentinel line + rc=1 → sentinel appears
    ONLY as `  | M3 GATE OK`, anchored count 0); run5 T-relay-rc
    (multi-line rc bytes carrying the sentinel → malformed-rc death,
    bytes relayed ONLY prefixed, anchored count 0).
  - .loop/m3-task7r62-tooth-respawn.log — 8/8: A lingering-old-pid
    never accepted (returns 1, no pid); B old-dies-then-new → NEW pid
    accepted; C old-dead-but-stale-pid-echo never accepted; D
    no-oldpid back-compat; rig_proc_pid single/multi/absent grammar;
    malformed-oldpid guard.
  - .loop/m3-task6r62-tooth-timingblank.log — both SHIPPED variants
    (awk-extracted function bytes, never transcribed): positive
    control clean bytes PASS; trailing-blank / terminator-removed /
    missing-final-newline all DIE; full-parser positive on a real
    artifact; CORPUS VALIDATION 5/5 genuine timing artifacts accepted
    (g01.au-dev-tim.{1,2}, g01.dev-tim, opk-tim@900, valve-tim@120) —
    zero false rejections (whitelist rule step 4).
  - In-run standing teeth T1-T10 all green through the new file-byte
    timing path (donecheck log step 5).
- **Freeze manifest + anchor (documented same-commit discipline)**: 6
  producers re-pinned — riglib.sh 0b4cba00…, check-device-opk.sh
  d07afaa7…, check-device-render.sh 42013be1…, judge-render-timing.js
  e034539d…, check-device-audio.sh 2ceace21…, verify_m3.sh NORMALIZED
  row 160a5bec… (excl. the ^MANIFEST_SHA256= line, per both headers);
  MANIFEST_SHA256 literal → 578bfbd5…. STATUS TRUTH: the two edited
  reviewed-go producers (check-device-render.sh,
  judge-render-timing.js) flipped to arc-pending with
  iter62-class-completion-scoped-confirm-pending cites (new bytes are
  unreviewed; refusal count 9 → 11 is the designed outcome); audio →
  …+iter62-probe-order-r3-scoped-confirm-pending; opk/gate →
  …+iter62-…-r3-scoped-confirm-pending. Direct self-check:
  `SELF-CHECK 23/23 + ANCHOR GREEN`
  (.loop/m3-task7r62-manifest-selfcheck.log).
- **Run ledger (caps held)**: 1 paced device invocation of cap 3
  (internal attempts=2 of ATTEMPTS_MAX=2 — attempt 1's single skip is
  the registered transient class, honestly logged); 1 arm rebuild
  (docker serial, pre-registered — 5 RIG_SCRIPTS members changed); 5
  host gate invocations of cap 6; host tooth harnesses only otherwise.
  No refutation shape fired.
- **Zoom-out (HARD RULE 8)**: all three items are class-level. The
  relay chokepoint turns "which output sites can spoof a sentinel"
  from an enumeration problem into a construction guarantee (any new
  relay site routes through relay_lines or reopens a documented
  contract — same family as iter-60's sentinel lockout: trust signals
  either mechanically derived or mechanically refused). The
  true-respawn poll closes the "liveness poll satisfied by the OLD
  process" class for every current and future frontend-cycling site
  via ONE shared riglib body. The timing-judge conversion COMPLETES
  the iter-61 "$() trailing-newline normalization" class — all three
  producer-output read sites (audio judge, pack verdict, timing judge)
  now read file bytes with generated-reference terminator asserts; the
  pattern is the reusable instrument. The probe move names and fixes
...
## iter 84 — 2026-07-19 — M4 hardening RESULT: mixer-rig round 1 CLOSED — stop witnesses + exposure pins + goldens-snd FOLDED, both cold checks green

**DONE.** Cold done-checks (both §7#1 detached + bounded foreground
polls): `bash port/gfx/check-mixer-fidelity.sh` → `MIXER FIDELITY OK
(goldens=11 diff=bit-identical maxvoices=9 steals=2 s01stops=4)`, exit 0,
FIRST post-commit run (.loop/m4-mixrig84-donecheck.log, RC-MARKER 0);
`bash port/sim/check-ai-live.sh` → `AI LIVE CONFORMS`, exit 0
(.loop/m4-mixrig84-donecheck2.log, RC-MARKER 0 — the fold-coordination
regression the brief demanded: the m4 manifest's cpu-set pins hold with
the non-cpu s01 row present). Teeth ALL fired
(.loop/m4-mixrig84-teeth.log; probes .loop/m4-mixrig84-probes.log; vfx
judge corpus validation .loop/m4-mixrig84-vfxjudge.log).

**THE FOLD (driver decision executed; sha proof)**: port/goldens-snd/
is GONE. s01's trace + frozen stream + crafted generator moved
(git mv) into port/goldens-m4/ with BYTES UNCHANGED — sha256 before ==
after: stream `4229cdebcbc18edb1f62812feb017fdfc24b87011b3af7159c6745
01cd5d9126`, trace `26e0589a8ffe5d6af5f666c3afd4a8fd7519e1a8b18e0535de
fe57c767357651`, gen-s01-trace.js `344b8cbf1fff58be6eb1870fefe17d703c9
316d41657f3a2df6a744f547b5f8a`. The recorder/freezer TWINS
(record-snd.sh, freeze-stream-snd.js) are deleted — one golden home,
one iter-83-hardened codebase. Re-pin verification WITHOUT browser
runs (.loop/m4-mixrig84-fold.log): freeze-stream-m4.js re-freeze from
the ARCHIVED record-s01-{a,b}.json → `unchanged (byte-identical
re-freeze)` + verify-stream self-check STREAM MATCH 3600/3600 — the m4
freezer is output-equivalent to the dead twin, proven not assumed.
Manifest grammar extended MINIMALLY: id/name `^m[0-9]{2}$` →
`^[ms][0-9]{2}$` (s = scenario goldens, CRAFTED committed generators —
documented in the manifest comment + freezer header); record-m4.sh
gains the crafted-trace refusal (missing s-trace dies pointing at the
committed generator — gen-trace.js can no longer silently fabricate a
wrong scenario trace).

**Per-fix (triage → landed form)**:
- **H stop-arm witnesses**: check-mixer-fidelity.sh [4b] now judges
  the s01 stop stream against four FROZEN witnesses — (frame, sound,
  preceding-play ordinal + frame): 191/sbc/ord5@171 (NSG release) ·
  697/sbc/ord18@571 (NSG auto-122) · 701/furaloop/ord21@649 (hitdet
  FURAFURA) · 1419/furaloop/ord30@1181 (FURAFURA wake) — strict
  full-line schedule walk, whole-stream equality, id == 1000+ordinal
  re-derived. The mixer stop counter SPLITS matched/unmatched
  (snd_mixer.h stopsMatched/stopsUnmatched; `stops` total + the
  gfx_app summary grammar unchanged), both renderers report
  stopsm=/stopsu= and the check binds C == capped-ref (everywhere) ==
  unlimited-ref (<=8 goldens); measured split: ALL live stops matched
  (g05 12/12, s01 4/4, unmatched 0 everywhere) — pinned. Teeth: T-W
  arm-swap (release dropped + auto doubled; aggregates PROVEN
  preserved) → witness death; T-SPLIT/T4b id-skew → stopsm=0 stopsu=4
  (the split SEES de-routing; now a standing in-check tooth).
- **Exposure pins (review High #2, folded under the brief's M2)**: the
  per-golden EXPOSURE_PINS table (unlimited-ref maxvoices / C steals /
  stops / stopsm / stopsu for all 11) is measured-then-frozen; over-cap
  set pinned == {g06, m02} (peak 9, 1 steal each), aggregate pins
  MAXV_ALL==9 / STEALS_ALL==2 / 11 goldens. The comparison basis can
  never silently switch. Tooth T-EXP: g06 fed maxv=8 → death.
- **M2 producer pins + inventories**: 3-entry strict pin table
  (check-sim.sh ce0882be… / wrap-run.js b835b5f8… / verify-stream.js
  f4207234…; ai-live kit, same update discipline) + ORACLE_IDS/NAMES +
  M4_IDS/NAMES arrays asserted == BOTH manifests' FULL inventories
  (both directions) + the eval class killed (strict read_golden
  key=value parse, 2nd consumer of the record-m4 form). Teeth: T-PPIN
  flipped pin → refusal before any run; T-INV shrunk M4_IDS → length
  death pre-lock; T-INV2 renamed pin → both-directions death;
  PREFLIGHT positive (pins + inventories + all-11 strict parse green).
- **M3 isolation/freshness**: no-reclaim run lock
  (port/gfx/build/mixerfid.lock) + rm-before-produce + made() on every
  produced artifact (audio stage output, pack a/b + verdict files,
  per-golden trace/schedule/sim-out/run-JSON/PCM/verdict files,
  app-leg outputs, tooth artifacts).
- **M4 grammars**: judge_verdict_file — decision files judged from
  FILE BYTES: exactly 1 line, trailing newline asserted, exactly-one
  full-grammar + exactly-one resembling line (pack, snd-render,
  snd-ref); read_terminator the same for schedules (terminator = final
  line, exactly one SNDEV-resembling line); git-guard rc CASE-SPLIT.
  Teeth: T-PACK extra-resembling-line + torn-newline deaths (+
  positive); T-GIT non-repo rc 128 → loud death where the old
  conditional silently skipped the guard.
- **M5 x2-identity + refreeze (freeze-stream-m4.js)**: runA/runB must
  be distinct files (resolved path AND dev:ino — symlink alias dies);
  --refreeze on differing bytes requires old specVersion != current
  (CHECKSUM.md §8 mechanical; unparseable old file also refuses).
  Teeth: T-FRZ1 same-path + T-FRZ1b symlink-alias deaths; T-FRZ2a
  differs-without-refreeze death (control) + T-FRZ2b same-spec
  --refreeze REFUSED (sandbox freezer copy + nibble-doctored frozen
  file; tracked bytes untouched); T-FRZ3 id 'x01' grammar death;
  positive: genuine archived-runs re-freeze → 'unchanged'.
- **M6 path containment**: inherited whole via the fold (the iter-83
  freezer grammar now governs s01; the snd twins that lacked it died).
- **L reference assurance debt — REGISTERED EXPOSURE, not fixed (per
  triage)**: snd_reference.js has no analytical PCM fixture / browser
  lifecycle oracle, and models play-ids as 1000+playCount without
  howler load/reset allocations — candidate task-14 hardening; the id
  plane is opaque, internally consistent, and off the checksum
  surface.
- **vfx-seam residual (iter-83 flag, same class)**: check-vfx-seam.sh's
  check-sim leg now rm's the g07/g08 bridge artifacts up front and
  pins the deterministic COLD 10-line shape (CPU goldens x2 STREAM
  MATCH + 2 absent-lines; ORACLE_CPU_IDS pinned into the inventory
  block). Regression = the LEG's judge only, validated by a harness
  built from the modified script's OWN bytes against the GENUINE
  cold-shape check-sim log from the ai-live run (PASS, zero false
  rejection) and a warm-doctored copy (death) —
  .loop/m4-mixrig84-vfxjudge.log. The full vfx matrix was NOT re-run
  (justified: the 10 component legs' code path is byte-untouched; only
  vfx_judge_log's else-branch + the pre-leg rm changed; a composed run
  costs hours for zero new coverage).

**Run ledger (honest; PROCESS §2)**: mixer composed cold runs 1/<=2
(green first try). check-ai-live composed runs **3 vs the <=2 cap —
overage recorded**: run 1 failed ONLY at its final contract guard on
the not-yet-committed fold artifacts (all substantive legs green — a
bootstrap-ordering fact this entry registers: a check that guards the
golden home clean cannot pass mid-iteration when the iteration itself
moves contract artifacts; resolution = commit-then-cold-run, result
entry amended into the same single commit, .loop/ is gitignored so the
commit content is identical either way); run 2 died mid-check-sim-leg
when `port/sim/calib/build/sim_host` VANISHED between g06 and g07 —
attributed to EXTERNAL interference: ~10 other agent sessions were live
on this host and something re-entered this checkout's shared calib
build scratch during the run (the leg log was later found rewritten by
a foreign process; quiescence was verified — ps/lsof empty, mtimes
stable — before run 3); run 3, quiescent, → AI LIVE CONFORMS. NOT a
fix round: zero project bytes changed between runs 1/2/3. CLASS NOTE
(zoom-out, HARD RULE 8): composed checks lock their OWN scratch but
check-sim.sh's $CAL/build is a shared unlocked resource across
check-ai-live/check-mixer-fidelity/check-vfx-seam AND any concurrent
driver session — single-writer discipline on this checkout is
currently procedural, not mechanical; flagged for the driver (a
checkout-level lock or driver serialization ruling would close the
class). Teeth/harness executions ~26 vs the pre-registered <=20 —
overage of ~6, ALL retries of registered teeth after harness-helper
bugs (a return-vs-exit fail() stub, a node -e quoting break, a grep
BRE-vs-literal mismatch), zero unregistered experiments. Probe
executions 8/<=24. Browser runs 0. No device, no docker, no pushes.

**TRIAGE-SCOPE FLAG FOR THE DRIVER (loud)**: review-82 carries a HIGH
the binding triage does NOT list — **GUARDON.c:56 ignores
as_shieldDepletion's return** (the depletion-break dispatch fix landed
only in GUARD.c; a shield breaking while being RAISED leaves the
victim in GUARDON — zero-coverage upstream-faithful gap, same class as
the iter-82 find). It is a sim-TU finding outside this iteration's
surface and belongs to the STATE.md-queued Tier-B sim-TU round — it
must NOT fall through the cracks when round 2 runs with the complete
diff (d0927fa plus this iter-84 commit). (The triage's other unlisted
High — dynamic
exposure basis — IS closed this iteration, see above.)

**Zoom-out (HARD RULE 8)**: (1) the witness pattern (exact
event-stream equality against frozen tuples instead of aggregate
counts) is the whitelist-grammar rule applied to EVENT STREAMS — any
future scenario golden gets witnesses for free via judge_witnesses;
(2) the matched/unmatched split turned an invisible no-op class
(stops that route nowhere) into a pinned observable on BOTH sides of
the differential; (3) the fold closed a whole duplicate-codebase class
(twin recorder/freezer) — the iter-83 hardening now covers s01 with
zero new code; (4) the ai-live kit (pins + inventories + eval-kill +
