...
| D48 | **A14 second half, 2026-08-25 — ITALIC MENU TEXT HAS ONLY ONE SIZE.** The FOH's display face is now the browser's own Arial, read out of the VFXGLYPHS1 atlas: upright text draws atlas font 0 ("900 40px Arial") and italic text draws font 3 ("italic 700 70px Arial", which is upstream's own menu weight). The atlas holds each spec at exactly one size, and `scale` became an integer upscale of the glyph mask. Font 0's ink cell is 9 device px — precisely what the retired 6x9 face occupied at scale 1 — so `up = scale` is a box-for-box drop-in and every upright string simply narrows to 0.67-0.88 of its old width. Font 3 has no such luck: it is 1.75x font 0, so its caps already stand 12 px where the old face stood 9, and `up = 1` is the only integer that fits anything (`up = 2` would set the mode titles 24 px tall inside an 18 px budget, against 22 px menu bars). **Consequence, measured and accepted:** italic scale 1 (the menu bar labels) and italic scale 2 (the mode titles, the CSS header, READY TO FIGHT) now render at the SAME height, where upstream sets its title larger than its bars ("italic 900 48px" over "700 35px"). Bar labels grow 1.20-1.44x, titles shrink to 0.61-0.71x; nothing overflows any screen's budget, the tightest fit being "TARGET BUILDER" at 123.7 px in roughly 128 px of bar. Restoring the size step needs a second italic capture in the atlas, which is a data-plane change, not a renderer one. Chase's acceptance playthrough is the authority; `foh_text2` in foh_font.c is the one place the metrics move. |
| D49 | **A14 second half, 2026-08-25 — MENU TEXT IS DRAWN AT BINARY COVERAGE, NOT THE BROWSER'S ANTIALIASING.** The atlas carries real antialiased alpha masks, and the HUD composites them exactly as captured. The FOH does not: `gfx_glyph_text_menu` collapses every mask sample to 0 or 255 at the halfway point. **This is not a shortcut, it is what keeps the menus provable.** Every FOH witness reads text off a rendered frame by OVERDRAWING it — draw the claimed string, in the claimed colour, at the claimed spot, and require zero changed pixels — and that instrument is exactly as strong as compositing being idempotent. Source-over at partial alpha is not: painting the same antialiased glyph twice darkens its fringe. Drawn antialiased, the instrument fails on frames that are in fact correct; measured, check-controls-labels.sh reported 388-474 px of "difference" on labels it was looking straight at. At binary coverage the property holds exactly and the check passes. The port also keeps the crisp look both hand-authored faces had on a 240x240 panel, which is the same reason those faces were binary to begin with. The HUD is untouched — nothing overdraws the HUD. |
