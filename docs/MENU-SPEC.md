...
| D57 | **A26/D53 + A45, 2026-08-26 — THE TARGET BUILDER RESUMES INTO ITSELF, BECAUSE ITS DOCUMENT NOW SURVIVES THE LID.** A45 T4 mapped `FOH_TBUILD` to `FOH_MENU_TOP` in `foh_persist_resume_target`, and the reason was a good one: the builder holds an UNSAVED document that was not persisted, so resuming into it would have presented an empty editor and read as *"my work is still here"* when it was gone — a refusal to lie rather than a limitation. **The honest way to remove a refusal is to make the statement it refused TRUE**, so the document is now persisted and the redirect is gone. It costs NO new format and NO persist-record version bump: the document is an `MlkStage`, A45 T1's `mlk_encode` already turns one into a share code, and it is published as `tbdoc.mlstage` through A45 T2's exact three-line contract (`MLSTAGE1` / code / `SUM`) and `foh_persist_publish`'s atomic tmp->fsync->rename — the same artifact and the same writer the SD slots use, validated on read with its SUM like any other. **THE REFUSAL DID NOT DISAPPEAR, IT MOVED TO WHERE IT CAN BE CHECKED:** `foh_persist_resume_target` is a pure domain map and cannot know whether the write succeeded, so `tdev_hibernate_check` publishes FIRST and **downgrades the resume to the menu top when that fails** — an armed builder resume with no document is precisely the lie the original redirect existed to prevent, and a full card or an `errors=remount-ro` flip is a MEASURED risk on this journal-less vfat. The file is CONSUMED at resume: it means *"the work you had when the lid closed"*, so a later ordinary visit must not resurrect it. MEASURED on the device: a write+fsync to `/mnt` costs 22 ms at 2 KB and 33 ms at 160 KB, inside the ~100 ms `powerdown schedule 0.1` grace that already carries the settings save. Proved by `port/foh/check-hibernate.sh` leg [5b], which asserts BYTE-IDENTITY of the document across park->hibernate->resume->hibernate (a resume that silently reset to D51's template passes every log line and fails that), and carries a dead-leg guard proving the parked document really differs from the untouched template | §11 |
| D58 | **A26/D53 + #29/#30, 2026-08-28 — A RESUMED MATCH OR TARGET RUN DOES NOT REPLAY `READY--GO!`, BECAUSE IT IS NOT STARTING.** Upstream draws the Ready--Go! banner from `dVfx/start.js`, spawned when a match begins, and the port carries it verbatim on every ordinary launch. A RESUMED match is set up FRESH first — boot, `sim_setup_match_ports`, the options plane — and only then has the played state written over it, because that is `ss_load`'s precondition. That set-up is not silent: it spawns the banner, and the banner instance lives in the RENDERER (`gfx_vfx.c`'s `g_q`), which a SIM restore does not reach. **MEASURED, reported by the owner on the first build where the resume otherwise worked end to end: a match 6562 frames old came back counting down from READY.** Upstream has no lid and therefore no opinion here, so this is the port's own feature behaving correctly rather than a departure from ground truth — but it is registered because it is a VISIBLE difference between two launches of the same match. The whole vfx queue is dropped at the restore, not the banner by name: every instance in it belongs to the match being discarded, and naming one would leave the next to be found by a player. **The count is LOGGED and asserted NON-ZERO**, so a seam that silently stops dropping fails instead of passing by doing nothing — the set-up spawns 3 today. RENDER-ONLY: the vfx plane is not on the CHECKSUM.md surface, so no stream moves and a FRESH match still shows the banner (asserted both ways). Proved by `port/foh/check-match-resume.sh` leg [7] and its twin in `port/foh/check-target-resume.sh` [2] | §11 |
| D59 | **A45/D52 + #30, 2026-08-31 — A CUSTOM TARGET STAGE KEEPS A PERSONAL BEST, AND FINISHING ONE NO LONGER KILLS THE APP.** Upstream's `targetRecords` is FIVE ROWS OF TWENTY (`targetplay.js:40`) and its cookie loader walks `j < 20` (`:156-158`): a custom slot's best time is stored exactly like an authored stage's, because `targetStagePlaying` is ONE index over both families (`main.js:1442-1445`). D52 widened this port's LAUNCH domain to that same 0-19 and the RECORD plane did not follow, which cost two bugs the owner found by playing. **MEASURED, reported: "I just finished breaking all the targets on a custom target stage and it crashed immediately"** — `foh_persist_record_update` bounds-checked `tstage < 10` and `gfx_fatal`ed on a journey the port's own menu offers. The second was silent: target-select's PERSONAL BEST indexed `targetRecords[p1Char][tssCursor]` whatever page it was on, so every CUSTOM slot displayed the AUTHORED stage's time as though the player had set it there. Both are the same root cause and both are fixed at one accessor (`foh_persist_record_get` / `foh_state_record`), which takes upstream's own 0-19 index. **The plane splits on the WIRE and only there:** the format indexes a row with a SINGLE digit and requires every row of a field to be present in order, so widening `rec` in place would have needed two-digit indices AND made every existing file fail its own grammar, resetting the player's settings and records — so the custom half is the appended key `crec`, which is the format's own documented extensibility path (absent in an old file and defaulted; no version bump, `MLFKPERSIST7` is still the last). The device whitelist grows from 87 lines/1927 bytes to 137/3227 and walks the new rows by position like the old ones. Proved by `check-persist-table.sh` (the fixture's crec values are deliberately disjoint from rec's, so a build that wrote one plane into the other fails on bytes) | §11 |
| D60 | **2026-08-31 — THE OPTIONS>AUDIO SLIDERS ARE ON THE BUS.** `snd_bus_set()` had existed, been documented and been exercised by `foh_snd_witness.c` since the mixer landed, and had **ZERO callers in the product**. `foh.c`'s audio arm wrote `FohState.masterVolume`, `foh_persist` saved and restored it, `foh_render` drew the bars moving, and nothing ever reached the mixer. **MEASURED, reported by the owner: "I don't think changing the music slider in the audio menu does anything. check the sounds slider too"** — both were dead, for the same reason, in the same way, and this is the SECOND instance this month of a seam that was built, tested and never connected (the first cost the whole match resume, 2026-08-28). The push is a RATIO, not a level: SNDPACK1's packed gains already carry the 0.5/0.3 shipping defaults, so pushing a raw level would apply the default twice and a half slider would be a quarter. Pushed on CHANGE from the FOH tick, and once at boot from the persist-apply site — because a `--direct-match` run and a lid RESUME both launch without ticking the FOH once, so a resumed match would otherwise have played at the shipping default however the player had set it | §4 |
| D62 | **A26/D53 + D57, 2026-08-31 — THE TARGET BUILDER RESUMES THE WORK IN PROGRESS, NOT ONLY THE WORK COMMITTED.** D57 made the DOCUMENT survive the lid and `check-hibernate.sh` leg [5b] proves it byte-for-byte. What it did not keep was everything the builder holds in `FohState`: the crosshair, the tool in hand, the half-dragged platform, the polygon with four vertices down and a fifth tracking the cursor. **MEASURED, reported by the owner: "resume for target test builder doesn't bring back what you were in the middle of drawing ... or the cursor position."** An uncommitted polygon is the ONLY thing on that screen which is not also in the document, so keeping the document and dropping it kept the wrong half. The view rides in its own file (`tbview.dat`) beside the document rather than in ~90 persist rows: none of it means anything to any other screen, the builder already owns a file with an atomic publish and a SUM, and the persist plane's every row is hand-declared, domain-checked and pinned by a positional device whitelist. Same rules as the document — validate on read, SUM before parse, refuse by name, CONSUME at resume — and its failure is explicitly NOT the document's: a resume with the stage and a default crosshair is worth having, one refused because the crosshair could not be written is not. The view is restored ONLY if the document was, since a half-drawn polygon on D51's template is a shape on a stage the player was not drawing it on. Proved by `check-hibernate.sh` [5b], which asserts byte-identity of the published view across park->hibernate->resume->hibernate and carries a dead-leg guard proving the parked view differs from a fresh builder's | §11 |
